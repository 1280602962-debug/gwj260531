...
A strict dual-target evaluation must distinguish four ligand states: **dual-active**, **A-selective**, **B-selective**, and **neither** (Figure 1A). A-only and B-only ligands are experimentally selective single-target controls: already potent on one target, they can produce plausible scores in that pocket while lacking corresponding activity on the other. The computational endpoint therefore asks whether docking discriminates dual-actives from the matching single-target selectives in both directions. Zhou, Li, and Hou benchmarked dual-kinase docking against noninhibitors.[9] Building on that setting, we compare directional selectivity comparisons with Dual versus neither on the same scores. Dual versus neither, scored on experimental inactives, is reported as a formulation contrast. A balanced four-state panel is limited by the need for paired measurements and enough selectives on both arms.
...
Here, we ask whether benchmark formulation changes the apparent evidence for dual-target recognition. We construct DualFourClass-Bench as a four-state panel with two directional primary tasks—dual versus A-only scored in pocket B, and dual versus B-only scored in pocket A (Figure 1B)—and summarize the weaker of those two arms as worst-arm AUROC (`summary_min`). Holding a single-target score channel fixed, we change only the experimental-state comparison to test how task definition itself changes the performance judgment; Dual versus neither under the two-pocket mean score is reported as a descriptive reference. We then test how far the observed discrimination can be explained by ligand chemistry or assigned to the intended pocket under alternative ligand panels, activity-aggregation rules, and receptor structures. This study examines how different both-end experimental-state comparisons change the interpretation of dual-target docking performance, and how far the observed discrimination can be explained by ligand chemistry or target structure.
