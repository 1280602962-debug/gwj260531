...
Instantiating that four-state data set is hard in public data. DualFourClass-Bench retains all four experimental states, but the prespecified primary evaluation is two directional pairwise discriminations (dual versus A-only and dual versus B-only), not a four-class classifier. Single-target virtual-screening benchmarks can rely on mature active/decoy construction.[5,6] A strict dual-target benchmark instead requires the *same* compound to carry comparable experimental measurements on both targets, and further requires A-only and B-only ligands with a clear selectivity gap. A usable experimental panel therefore needs not only enough dual-actives, but also selective hard negatives that are adequate in count and potency range on **both** arms.
...
First, we audit candidate target pairs in public activity data and, from paired experimental potencies and selectivity gaps, assemble evaluation panels of **dual-active, A-only, B-only, and neither** ligands. The same audit quantifies the actual supply of strict four-state benchmarks rather than assuming that every candidate pair meets the same data requirement. Neither is retained to describe the full state space; the prespecified primary endpoint uses only dual versus A-only and dual versus B-only. Results are reported on the multi-pair panels that survive this supply audit (construction rules, receptors, and docking parameters in Methods).
