...
Here, we ask whether benchmark formulation changes the apparent evidence for dual-target recognition. We construct DualFourClass-Bench as a four-state panel with two directional primary tasks—dual versus A-only scored in pocket B, and dual versus B-only scored in pocket A (Figure 1B)—and summarize the weaker of those two arms as worst-arm AUROC (`summary_min`). The eight pairs are built under one harvest protocol with pair-specific quotas (Table 1) and are used to test consistency across systems rather than as eight independent replicates. We then test how far the observed discrimination can be explained by ligand chemistry or assigned to the intended pocket under alternative ligand panels, activity-aggregation rules, and receptor structures. The contribution is an evaluation-design and empirical diagnosis, not a new scoring algorithm.
