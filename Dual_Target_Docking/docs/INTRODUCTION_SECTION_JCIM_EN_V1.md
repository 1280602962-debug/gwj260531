...
Here, we ask whether benchmark formulation changes the apparent evidence for dual-target recognition. We construct DualFourClass-Bench as a four-state panel with two directional primary tasks—dual versus A-only scored in pocket B, and dual versus B-only scored in pocket A (Figure 1B)—and summarize the weaker of those two arms as worst-arm AUROC (`summary_min`). The original primary set was the 2026-07-23 four-pair freeze; five census-qualified ordinary pairs were later added to the same stack, and PIK3CA/PIK3CB was withdrawn, leaving eight primary rows. We then test whether the observed discrimination is retained under alternative ligand panels, activity-aggregation rules, and receptor structures. This is collection completion, not a claim that eight pairs were frozen before scores were seen.
