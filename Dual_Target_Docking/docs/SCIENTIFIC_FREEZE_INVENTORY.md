...
|---|---|---|---|---|---|
| C1 | ZH/EN incremental |Δ| disagreed, and deposited ECFP CSV 0.0112 was not reproducible in the freeze environment. | Baseline ZH Discussion 0.023 vs EN 0.011; freeze rebuild + old `fit_ecfp4_models.py` on the identical master both give max \|Δ\| = 0.0234 at PPARA/PPARD D vs B (rdkit 2026.03.6, sklearn 1.9.1). Table 2 unchanged. | Manuscripts, SI Table S5, ECFP CSVs, Figure 3. | Accept the rebuilt 0.023; do not chase 0.0112. | EN/ZH/SI/Figure 3/pack tables use freeze incremental; leftover 0.0112 files are marked historical. |
...
| R4 | EGFR five-seed previously isolated as a different-box AUROC table (complete-case 28/38/32, D vs A 0.6607). | Corrected-box per-seed scores restored; recomputed with current activity-eligible labels 28/37/32/12. Production seed matches Table 2 (0.3237). | Figure 5C; Table S7d. | Use corrected-box five-seed as the current robustness comparison. | Restored `scores_vina_mode1_EGFR_corrected_box_fiveseed.csv`; `five_seed_summary_min.csv` comparable=1. |
