...
现有研究已经表明单靶选择性配体会影响双靶筛选结果，但尚缺少对同一套对接评分在不同双端实验状态比较下如何改变性能解释，以及这些判别在多大程度上来自配体化学或靶点结构的系统评价。本研究基于双端实验活性构建四状态评价框架，以 dual 对 A-only、dual 对 B-only 两个方向性比较为主要评价，并将 dual 对 neither 作为不同实验状态定义下的参照。在保持对接评分不变的条件下比较这两类任务，再引入不使用受体结构的配体化学基线，并用口袋对应关系、受体结构和独立对接实现检验剩余判别是否与靶点结构信息相一致。本文关注的是双靶对接评价结论本身的来源，而不是提出新的打分函数。
...
对满足数据供给条件的靶对进一步检查后，部分体系因共同高通量测定背景、靶点类型、结合位点性质或缺少合适的人源实验结构而不适合纳入统一非共价对接评价。最终纳入主要评价的靶对为 PIK3CA/mTOR、AChE/BChE、EGFR/HER2、F2/F10、JAK1/TYK2、JAK1/JAK2、PPARG/PPARA 和 PPARA/PPARD（Table 1）。EGFR/HER2 的严格 B-only 样本供给仍然有限，因此保留为双向选择性数据受限的评价体系。四状态分类要求同一配体在两个靶点上均具有实验测量，双端实验测量覆盖和双向选择性样本数量共同限制了严格四状态评价集的规模。

### 3.2 实验状态定义与方向性对接评价

固定同一靶点评分、仅改变对照配体的实验状态后，部分靶对的表观判别发生明显变化。EGFR/HER2 使用靶点 A（EGFR）评分时，dual-versus-B-only 的 AUROC 为 0.430，将负类替换为双端低活性的 neither 后升至 0.808，差值 0.378 [0.205, 0.547]；另一方向上的负类差异较小。JAK1/TYK2 使用靶点 A（JAK1）评分时，dual-versus-neither 与 dual-versus-B-only 的差值为 0.444 [0.263, 0.620]。该负类效应在不同靶对和评价方向中并不一致：AChE/BChE、JAK1/JAK2、PPARG/PPARA 和 PPARA/PPARD 在相同评分通道下的差值较小，95% 置信区间均包含 0（Figure 2C；Table S4）。
...
采用 GNINA 1.3.2 独立生成姿态和评分后，EGFR/HER2 的 dual-versus-neither AUROC 为 0.783 [0.610, 0.922]，而方向性 \(\mathrm{summary}_{\min}\) 为 0.220 [0.109, 0.343]；JAK1/TYK2 同样呈现该模式，dual-versus-neither 为 0.705，方向性 \(\mathrm{summary}_{\min}\) 为 0.317 [0.183, 0.463]（Figure 4A；Table S9）。更换姿态生成和评分流程后，这两个靶对中 dual-versus-selective 与 dual-versus-neither 之间的评价差异仍然存在。

PPARG/PPARA 在主要 Vina 评价中是唯一 \(\mathrm{summary}_{\min}\) 置信区间完全高于 0.5 的靶对（0.649 [0.504, 0.751]），但在同姿态 RTMScore 重评分下降至 0.369 [0.233, 0.475]，GNINA CNN 重评分降至 0.500，且在未使用池留出集中降至 0.535 [0.350, 0.717]（Table S7；Table S9）。不同 Vina 随机种子及 PIK3CA/mTOR 的 exhaustiveness 设置产生的数值波动相对有限（Figure 4C；Figure 6C；Table S9）。

### 3.5 方向性评价的标签与样本组成敏感性

改变活性阈值（\(\theta=5.5\)、6.0、6.5 及严格 6.5/5.5）后，样本较充足靶对的 \(\mathrm{summary}_{\min}\) 数值变化较小（AChE/BChE 稳定在 0.606）。当单侧选择性样本量明显减少时，估计波动增大：EGFR/HER2 在严格 6.5/5.5 条件下 B-only 仅 7 个，\(\mathrm{summary}_{\min}\) 降至 0.324（Figure 6A；Table S3）。将重复活性记录的汇总方式由最大 pChEMBL 改为中位数后，标签一致率保持在 93.6%–100%，方向性汇总指标的最大绝对变化为 0.023（Table S3）。在原冻结已打分配体上，人源单蛋白高置信记录筛选未改变四状态标签与方向性 AUROC；普查后五对未纳入该视图（Table S3）。
...
按本研究的外部评价准入条件筛选后，没有任何靶对同时满足 dual、A-only 和 B-only 的样本量及来源多样性要求，均未进入外部对接流程（Figure 6D；Table S11）。按配体发表年份进行时间切分同样受到双向选择性样本供给的限制。以 2018 年为界，没有靶对在时间切分测试集中满足双向评价所需的类别样本量，因此未能构建出可同时独立评价两个方向的时间切分测试集（Table S12）。在本研究采用的准入标准下，当前公开数据尚不足以为这些靶对构建独立的双向四状态外部评价集。
...
排入 Supporting Information 的核实表为 **Note S0**（PIK3CA/PIK3CB 受体身份失败）与 **Tables S1–S13**（软件与种子、对接盒子与共晶 RMSD、阈值与 pChEMBL 聚合、固定评分负类、ECFP4 增量、对应/非对应口袋、未使用池留出集、PIK3CA/mTOR 晶体替换、独立 GNINA 与五种子 Vina、文献簇与可检测效应、BindingDB 外部准入、2018 年切分计数、EGFR/HER2 操作点）。历史工作表 S1–S54 的合并对照见 `data/manuscript_lock/SI_TABLE_MERGE_MAP_v1.csv`。
