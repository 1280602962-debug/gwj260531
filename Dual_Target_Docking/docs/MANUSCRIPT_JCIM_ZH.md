...
主要分析是固定同一靶点评分通道后的两个方向性 AUROC，以及两者较低值的描述性汇总 \(\mathrm{summary}_{\min}\)。在同一评分通道下另做 dual–neither 比较，用于对照而不是替换方向性主指标。配体化学基线、对应/非对应口袋、受体替换、独立姿态生成、标签敏感性、内部留出和时间切分用于限定解释范围；BindingDB 与 PubChem 清点用于判断外部评价集是否形成。这些补充分析不构成第二套主结论。
...
为考察活性数据处理方式的影响，做了三类彼此独立的检查，均保持评价集成员和对接评分不变（Table S3）。第一，在 2026-08-26 同日 API 快照上，对原有三个靶对分别用最大和中位 pChEMBL 重标；该快照与生产标签所用冻结缓存不是同一数据版本。第二，在同一 API 快照覆盖范围内，按人源、单蛋白等数据库字段规则做一次高置信自动筛查，不是逐篇阅读原文。第三，新增五对另有基于 ChEMBL 37 数据库转储的最大/中位 pChEMBL 重标；该结果与同日 API 快照不是同一数据版本，也不替换 Table 2。
...
在评价集对接前，对各主受体开展共晶配体重对接，以评价搜索设置对实验结合构象的覆盖能力。每个共晶配体最多输出 9 个姿态。搜索覆盖取全部保存姿态中的最低重原子 RMSD，并另报 top-1 RMSD，以区分姿态生成与评分排序。AChE 4EY7 与 TYK2 3LXP 实际保存 8 个姿态，其余主受体保存 9 个姿态，因此该覆盖指标不是统一的 best-of-9。最低 RMSD 低于 2.0 Å 只说明搜索能够产生近天然构象，不表示评分排序或选择性预测可靠。新增八个受体的近天然判断以化学对应 RMSD 为准（Table S2c）；原有六个受体沿用其已有 CalcRMS 或生产 QC 表。EGFR 3POZ 为后来重建的 QC。坐标匹配历史对照见 Table S2b。[8]
...
通过改变活性阈值及重复活性记录的汇总方式重新确定实验状态，并在保持评价集成员和对接评分不变的条件下重复主要分析（Table S3）。对于具有足够剩余候选配体的靶对，在排除主评价集成员后构建未使用池留出集。留出配体使用固定抽样规则，并限制单一 Bemis–Murcko 骨架的过度重复。该分析使用不同配体重新计算主要方向性 AUROC，用于评价结果对评价集成员组成的敏感性。由于这些配体仍来自同一数据来源，该分析属于内部稳健性检验（Table S7）。另通过多个固定随机种子重复主要 Vina 对接，并在 PIK3CA/mTOR 中比较不同 exhaustiveness 设置，以评价搜索随机性和搜索强度对结果的影响（Table S9）。备选种子按完整病例计：AChE/BChE 生产种子 n_complete 为 95，其余四种子为 89–90。新增五对另按五个种子都成功打分的配体取交集，与完整病例并列（Table S9e）。该交集只覆盖这五对，不外推为八对。完整病例估计的是该种子实际得到分数的成员，交集估计的是成员固定时的搜索波动。缺失评分不作为低活性。
...
DUD、DUD-E 和 LIT-PCBA 等基准已经表明，对照来源会改变虚拟筛选评价；[5–7,12,13,21] Kinase-Bench 则面向激酶选择性识别。[22] 本文回答的是另一问题：在已测两端活性的化合物中，更换实验状态比较如何改变对接判别的解释。
