...
分子对接常用于双靶点虚拟筛选中候选分子的排序与优先选择。然而，在回顾性评价中，对照化合物既可以是两个靶点均无活性的双端低活性分子，也可以是仍对某一靶点保持强活性的单靶选择性分子。本研究在八个人源靶对上建立包含 dual、A-only、B-only 和 neither 的四状态评价体系。在固定同一靶点评分通道的前提下，仅改变对照类别即可显著改变表观对接判别：EGFR/HER2 中 EGFR 口袋评分对 dual 与 B-only 的 AUROC 为 0.430，而在相同评分通道下与 neither 比较则升至 0.808（差值 0.378 [0.205, 0.547]）；JAK1/TYK2 同样出现类似差值（0.444 [0.263, 0.620]），且独立 GNINA 姿态生成再现了这一任务差异。基于骨架分组的仅配体 ECFP4 基线在多个方向上达到与对接相当的判别力，引入对应口袋对接评分后的最大 AUROC 增量仅为 0.023。在结构归因方面，仅 EGFR/HER2 和 AChE/BChE 主面板的对应口袋较弱方向评分优于非对应口袋，但在未使用的留出集分子中，对应口袋优势未能稳定重现。上述结果表明，在具有双端实验活性的化合物集中，双端低活性对照可能掩盖对单靶活性分子的判别不足；回顾性双靶评价应分别考察两个方向的选择性对照，并结合配体化学基线与口袋对应性分析以厘清判别来源。
...
现有研究已经表明单靶选择性配体会影响双靶筛选结果，但在已有双端实验活性的数据中，评价任务的定义如何改变对接性能的解释，仍缺乏系统考察。基于此，本研究重点回答三个问题：第一，固定评分通道时，改变所比较配体的实验状态是否显著改变对双靶对接性能的判断；第二，表观观察到的判别能在多大程度上由不使用受体结构的配体化学特征解释；第三，剩余判别是否能够稳定归因于相应靶点的口袋结构特征。本研究据此考察双靶对接性能评价的含义及其判别来源。
...
固定相同评分通道后，改变实验状态比较所产生的 AUROC 差异在不同靶对和评价方向之间差别较大，不同靶对和评价方向中的差异幅度并不一致。其中 EGFR/HER2 和 JAK1/TYK2 出现了较为明显的差异。EGFR/HER2 使用靶点 A（EGFR）评分时，dual-versus-B-only 的 AUROC 为 0.430，将对照类别替换为双端低活性的 neither 后升至 0.808，差值 0.378 [0.205, 0.547]；另一方向上的差异较小。EGFR/HER2 的该差值在骨架簇区间 [0.168, 0.562] 和文献簇区间 [0.083, 0.529] 下仍保持排除 0；JAK1/TYK2 使用靶点 A（JAK1）评分时，两项比较的差值为 0.444 [0.263, 0.620]，其骨架簇区间为 [0.234, 0.633]，仍排除 0，但文献簇区间为 [−0.034, 0.682]，包含 0，显示不确定性增大（Table S4；Table S10）。固定评分通道的比较仍存在明显差异。AChE/BChE、JAK1/JAK2、PPARG/PPARA 和 PPARA/PPARD 在相同评分通道下的差值较小，配体水平 95% 置信区间均包含 0（Figure 2C；Table S4）。
...
为检验方向性判别是否具有靶点口袋特异性，Figure 5 与 Table S6 比较对应口袋与非对应口袋的 \(\mathrm{summary}_{\min}\) 差值。主评价集中仅 EGFR/HER2（0.170 [0.060, 0.280]）和 AChE/BChE（0.161 [0.037, 0.269]）两对的差值区间排除 0；其余六对的 95% 置信区间均包含 0。在七个未使用的留出集中，对应口袋与非对应口袋的 \(\mathrm{summary}_{\min}\) 差值区间均包含 0（差值介于 −0.079 至 +0.150；Figure 5B；Table S7）。对应口袋优势未在留出集中稳定重现。
...
采用 GNINA 1.3.2 独立生成姿态和评分后，EGFR/HER2 的 dual-versus-neither AUROC 为 0.783 [0.610, 0.922]，方向性较弱臂 dual-versus-B-only 为 0.220 [0.109, 0.343]；JAK1/TYK2 同样呈现该模式，dual-versus-neither 为 0.705，方向性 \(\mathrm{summary}_{\min}\) 为 0.317 [0.183, 0.463]（Figure 4A；Table S9）。在独立姿态生成流程下，类似差异仍可观察到。
...
改变活性阈值（\(\theta=5.5\)、6.0、6.5 及严格 6.5/5.5）后，样本较充足靶对的 \(\mathrm{summary}_{\min}\) 数值变化较小（AChE/BChE 稳定在 0.606）；当单侧选择性样本量减少时估计波动增大（Figure 6A；Table S3）。将重复活性记录由最大值改为中位数，以及高置信记录筛选，总体未改变方向性判别结果（Table S3）。
...
BindingDB 与 PubChem 按同一四状态规则清点八个靶对的双向供给，并在排除共享文献、重复结构和高相似分子后进行独立来源评估。没有任何靶对满足预设的独立外部评价准入条件，因此未进行外部对接（Figure 6D；Table S11）。按配体发表年份进行时间切分同样受到双向选择性样本供给的限制，未能构建出可同时独立评价两个方向的时间切分测试集（Table S12）。未使用池留出集与年份切分都只是内部敏感性，不作为外部验证。当前公开数据尚不足以为这些靶对构建独立的双向四状态外部评价集。
