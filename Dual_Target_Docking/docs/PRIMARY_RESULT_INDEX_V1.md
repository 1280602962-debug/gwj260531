...
| Table S2b 共晶 RMSD | 搜索覆盖（坐标匹配，历史对照） | 保存共晶姿态 | `cognate_qc/*_out_E8.pdbqt` 等 | `layer3_cognate_rmsd_v1.csv` |
| Table S2c 共晶 RMSD | 14 受体统一化学对应 | 同一套保存姿态，不重对接 | 同上 | `all14_cognate_rmsd_calcrrms_v1.csv`；Figure S4 用此表 |
