...
**第 4 段。** 删除“首先 / 随后 / 进一步”四步路线图（intro 只预告问题与研究路线，不预告实验步骤）。末句改为：本文关注评价结论的来源，而不是新打分函数。冻结日期、DualFourClass-Bench、PIK3CA/PIK3CB 撤回不进引言（provenance 放方法/局限）。
...
**2.3。** 原稿把“A-only 和 B-only ≥50”写成八对共同准入，不成立。改为：厚面板操作性门槛；EGFR/HER2 未过门槛而保留；五个普查靶对按预先声明门槛后加入；PIK3CA/PIK3CB 因 2WXF 身份错误撤回（Note S0）。Table 1 数字与 9V8H 肽脚注保持冻结表。
...
**4.5。** 保留样本、异质性、外部不可得、Vina 为主。补一句 PIK3CA/PIK3CB 撤回（Note S0）。删除“unused-pool holdout”英文夹杂。
