...
本文的核心不是“对接能不能识别双靶配体”的全称判断，而是：在公开可构建的四状态硬负面板上，对接分数何时还保留方向信号、何时被分子属性带走。K = 4 是供给审计冻结的评价集，不是对全部双靶任务的抽样。ChEMBL 作为对接主数据源是合理的：BindingDB/PubChem 计数核对未翻转厚面板门槛（Table S12），但 EGFR/HER2 在等式测定下仍达不到 ≥ 50，不能改写成厚面板。PIK3CA/mTOR 在单一受体实现下可见方向判别，但换晶后幅度对结构选择敏感，不得写成结构不变的 positive case。
...
PIK3CA/mTOR 是唯一在点估计上同时高于随机与最强描述符基线的靶对，但其 Δ 的置信区间仍含 0，且换晶体后受体依赖。文章能主张的是：对接增量高度对靶、对结构；评测协议必须同时报告硬负、分臂与混淆对照。GNINA 与 RTM 的姿态覆盖此前不对称，2026-08-24 已补做全 9 姿态公平重打（`GNINA_POCKET_MATCHED_BEST9_VERDICT_V1.md`）：GNINA 口袋匹配 summary_min 变化很小，姿态覆盖不是其结论偏弱的主要原因；三对上不超过同面板 Vina，PIK3CA/PIK3CB 上二者相近（均近随机，GNINA 略高但区间重叠，且该关系在 mode-1 时已存在）。GNINA 仍只是单一 CNN 通道对照，不构成三引擎公平赛马的正式主张。
