...
PIK3CA/mTOR 是唯一在点估计上同时高于随机与最强描述符基线的靶对，但其 Δ 的置信区间仍含 0，且换晶体后受体依赖。文章能主张的是：对接增量高度对靶、对结构；评测协议必须同时报告硬负、分臂与混淆对照。GNINA 与 RTM 的姿态覆盖此前不对称，2026-08-24 已补做全 9 姿态公平重打（`GNINA_POCKET_MATCHED_BEST9_VERDICT_V1.md`）：GNINA 口袋匹配 summary_min 变化很小且四对均不超过同面板 Vina，姿态覆盖不是其结论偏弱的主要原因；但 GNINA 仍只是单一 CNN 通道对照，不构成三引擎公平赛马的正式主张。
