...
Primary analysis used one unified θ = 6.0 label; the strict 6.5/5.5 rule was reserved for supply audit and construction gating. AChE/BChE and PIK3CA/PIK3CB were sampled under the strict gate (target 28 / 28 / 28 / 16; n_panel = 100). EGFR/HER2 (n_panel = 110) and PIK3CA/mTOR PM48 (n_panel = 48; constructed 18 / 14 / 12 / 4) had too few B-only ligands under the strict gate, so those panels were assembled under θ = 6.0 at the supply-audit stage. Cross-pair AUROCs therefore mix target-pair biology with panel-construction differences. Ligand–receptor jobs that failed to yield a score were dropped; n_scored can fall below n_panel (Table 1; Table S27). An expanded PIK3CA/mTOR panel (PM110) keeps all 48 PM48 ligands as a nested size check.
...
The worst-direction discrimination summary is \( \mathrm{summary}_{\min} = \min(\mathrm{AUC}_{D/A},\;\mathrm{AUC}_{D/B}) \). It is a conservative summary of two directional AUROCs, not a new docking score. Arithmetic, geometric, and harmonic means are aggregation sensitivities (Table S26). The single primary endpoint is pocket-matched Vina `summary_min` under unified θ = 6.0 (Table 2; PIK3CA/mTOR uses PM48). A prespecified RDKit panel (heavy-atom count, molecular weight, cLogP, and TPSA) was evaluated with the same directional workflow; the highest AUROC among them is a best single-descriptor reference (Tables 2, S28, S19). Dual versus neither (experimental inactives; `vina_mean`) is a descriptive, nonselectivity-controlled comparator; Dual versus all non-duals is an auxiliary formulation contrast on the same frozen scores (Table 3; Table S22). PIK3CA/mTOR neither n = 4 is flagged underpowered.
...
This study established DualFourClass-Bench to evaluate directional dual-target docking against experimentally defined A-selective and B-selective hard negatives. Across the four frozen target pairs, `summary_min` ranged from 0.430 to 0.692, and adding docking scores to ECFP4 produced only limited incremental improvement under scaffold-grouped cross-validation. Sensitivity to activity-aggregation rules did not change the main conclusions, whereas receptor replacement substantially altered apparent discrimination, including opposite-direction shifts on two PIK3CA-related pairs.

These results indicate that apparent dual-target docking discrimination is task-, chemistry-, and receptor-dependent; favorable scores in both pockets are insufficient evidence of dual activity. Dual-target virtual screening, and generative workflows that use docking as a downstream filter, should therefore consider selective hard negatives, ligand-level chemical controls, unused ligand pools, and receptor-structure sensitivity together. DualFourClass-Bench is intended to define the evidentiary boundary of docking-based dual-target recognition rather than to supply a universal docking ranking rule.
