...
Primary labels use the maximum available pChEMBL value. To test the sensitivity of the aggregation rule, ChEMBL assay-level pChEMBLs were re-fetched and re-aggregated with both maximum and median rules. This analysis compared the two aggregation methods without redefining panel membership, docking parameters, or Vina scores. Because of one cache/API inconsistency (`EH120_060` / CHEMBL24828), the API-refetched maximum aggregation for EGFR/HER2 (0.417) differs slightly from the frozen primary panel (0.430), so A4 values do not replace the Table 2 frozen primary results. Median aggregation changed 7/110, 1/95, 1/99, and 0/48 ligand-state assignments, corresponding to label agreements of 93.6%, 98.9%, 99.0%, and 100%, respectively (Table S29). Despite these limited label changes, the pair-level `summary_min` estimates changed only modestly (0.417→0.424, 0.606→0.629, 0.500→0.500, and 0.692→0.692), indicating that the main benchmark interpretation was largely insensitive to this aggregation choice. Assay-level heterogeneity remains, because pChEMBL values are not assay-equivalent. PIK3CA/mTOR remained unchanged at 0.692 under both API-max and API-median aggregation.
