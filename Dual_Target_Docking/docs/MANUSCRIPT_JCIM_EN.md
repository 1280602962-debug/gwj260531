...
Prior work has shown that single-target selectives affect dual-target screening. What remains less clear is how the evaluation task itself changes the interpretation of docking performance when both-end experimental activities are already available. We asked three questions. First, after the score channel is held fixed, does changing the experimental-state comparison change the performance judgment? Second, what discrimination can ligand chemistry obtain without receptor structure? Third, is remaining discrimination consistent with the corresponding pocket, and is that correspondence stable on existing internal controls?
...
The primary analysis comprised the two directional AUROCs at a fixed target score channel and their descriptive lower value, \(\mathrm{summary}_{\min}\). Dual-versus-neither at the same channel was a comparator, not a replacement for the directional endpoints. Ligand-chemistry baselines, matched versus mismatched pockets, receptor substitution, independent pose generation, label sensitivity, unused-pool holdouts, and year splits were used to bound interpretation. BindingDB and PubChem counts asked whether an external evaluation set could be formed. These supplementary analyses do not constitute a second primary result.
...
Before panel docking, each main receptor was redocked with its cognate ligand to assess whether the search settings could cover the experimental bound conformation. At most nine poses were retained. Search coverage used the lowest heavy-atom RMSD among saved poses; rank-1 RMSD was reported separately. AChE 4EY7 and TYK2 3LXP deposited 8 poses, and the other primary receptors deposited 9, so the coverage metric is not a uniform best-of-nine. A value below 2.0 Å indicates that the search could produce a near-native pose, not that rank-1 placement or selectivity prediction is reliable. Near-native calls for the eight added receptors use chemically mapped RMSD (Table S2c). The original six receptors keep their existing CalcRMS or production-QC tables. EGFR 3POZ is reconstructed QC. Coordinate-assignment values remain in Table S2b as a historical comparison.[8]
