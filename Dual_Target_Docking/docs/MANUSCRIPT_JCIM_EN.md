...
The primary analysis comprised the two directional AUROCs at a fixed target score channel and their descriptive lower value, \(\mathrm{summary}_{\min}\). Dual-versus-neither at the same channel was a comparator, not a replacement for the directional endpoints. Ligand-chemistry baselines, matched versus mismatched pocket scores, receptor substitution, independent pose generation, label sensitivity, unused-pool holdouts, and year splits were used to examine possible sources of discrimination and the sensitivity of the main findings. BindingDB and PubChem counts asked whether an external evaluation set could be formed.
...
Activity-processing checks were kept separate and all held panel membership and docking scores unchanged (Table S3). Aggregation sensitivity was evaluated using two archived activity sources. The 2026-08-26 ChEMBL API snapshot was available for EGFR/HER2, AChE/BChE, and PIK3CA/mTOR, whereas JAK1/JAK2, JAK1/TYK2, F2/F10, PPARG/PPARA, and PPARA/PPARD were reassessed using the ChEMBL 37 dump. Because these sources were not identical, the two checks were interpreted separately. A high-confidence human SINGLE PROTEIN field screen was applied to the API snapshot; it is an automatic filter, not paper-by-paper reading.

### 2.3 Pair screening and panel construction

**Pair-supply screen.** Candidate pairs came from one ChEMBL extract. Targets were restricted to human, single-component SINGLE PROTEIN records with both-end quantitative activity. Maximum pChEMBL was the representative value, and ligands were assigned to four states as in section 2.2. Bidirectional selective supply was counted under the strict 6.5/5.5 rule (active end \(\geq 6.5\), low-activity end \(\leq 5.5\); gray-zone ligands were excluded from that candidate pool). A pair was retained only if both targets had a human experimental structure, a defined noncovalent ligand-binding site, a cognate ligand that permitted docking-box definition, and enough dual, A-only, and B-only ligands to build a directional panel. The included pairs were EGFR/HER2, JAK1/JAK2, JAK1/TYK2, PIK3CA/mTOR, AChE/BChE, F2/F10, PPARG/PPARA, and PPARA/PPARD.
...
Before panel docking, each main receptor was redocked with its cognate ligand to assess whether the search settings could cover the experimental bound conformation. At most nine poses were retained. Search coverage used the lowest heavy-atom RMSD among saved poses; rank-1 RMSD was reported separately. AChE 4EY7 and TYK2 3LXP deposited 8 poses, and the other primary receptors deposited 9, so the coverage metric is not a uniform best-of-nine. A value below 2.0 Å indicates that the search could produce a near-native pose, not that rank-1 placement or selectivity prediction is reliable. Redocking was used only as a qualitative search-coverage check. Atom mapping and RMSD implementation were not uniform across the 14 receptors (Tables S2b and S2c), so the values were not compared quantitatively across receptor groups. Coordinate-assignment values remain in Table S2b as a historical comparison.[8]
...
The 95% confidence intervals for \(\mathrm{summary}_{\min}\) in Table 2 were estimated from 2000 ligand-level non-stratified percentile bootstrap replicates. Each replicate drew, with replacement, the original number of ligands from the pooled dual, A-only, and B-only set used in that pair’s directional analysis. Dual-versus-A-only and dual-versus-B-only AUROCs were then recomputed, and the smaller value was that replicate’s \(\mathrm{summary}_{\min}\). If a replicate lacked a required class, it was omitted from the interval. The 95% interval was the 2.5th and 97.5th percentiles of valid \(\mathrm{summary}_{\min}\) values. Point estimates were computed from the full analysis sample, not from bootstrap means. Dual-versus-neither AUROC intervals, and the dual-versus-all-nonduals intervals, used class-stratified percentile bootstrap.
...
Each of the 14 primary receptors produced at least one saved pose below 2.0 Å under its recorded QC table. That result is a qualitative search-coverage check, not a quantitative RMSD comparison across receptors. AChE 4EY7 and TYK2 3LXP saved 8 poses; the other primary receptors saved 9. EGFR 3POZ top-1 was 9.505 Å, with lowest saved-pose RMSD 0.760 Å. JAK2, PPARG, and PPARA failed the 2 Å top-1 cutoff; PPARA 6LXA top-3 was 7.848 Å (Figure S4; Table S2c).
...
Panel membership, experimental-state labels, receptor and docking-box definitions, per-ligand score tables, analysis tables, and the scripts used to regenerate the reported statistics and figures are available in the `Dual_Target_Docking` directory of the public repository at https://github.com/1280602962-debug/gwj260531. All numerical analyses can be reproduced from the deposited score tables; raw production pose archives are not required for regeneration of the reported statistics. Typeset Supporting Information contains Tables S1–S13.
