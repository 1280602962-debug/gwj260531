...
双靶对接评测被建成四类配体任务。对每一对靶标 A/B，配体按两端实测活性分为：dual（两端均强）、A_only（仅 A 强）、B_only（仅 B 强）、neither（两端均弱）。主问题不是“谁在某一口袋打分更高”，而是对接分数能否同时压住两条单靶硬负臂。因此主指标定为方向性判别：dual 对 A_only、dual 对 B_only 分别计算 AUROC，并以两臂最小值 `summary_min` 汇总。更弱的一臂决定该靶对上的可报告表现。

### 2.2 数据策展与靶对选择

活性数据取自本地缓存的 ChEMBL 靶标分子字典。每个配体–靶标对保留可用的最大 pChEMBL；缺少任一端实测值的配体不进入四类主分析。盐按 ChEMBL 规范写法处理，配体准备阶段再拆成最大片段。

四类标签采用两套规则，并在正文与敏感性分析中同时报告。**严格规则**（strict）：dual 为两端 ≥ 6.5；A_only 为 A ≥ 6.5 且 B ≤ 5.5；B_only 为对称定义；neither 为两端 ≤ 5.5；介于 5.5 与 6.5 之间的灰区配体不进入严格面板。**单阈值规则**（θ）：两端均 ≥ θ 为 dual，一端 ≥ θ 且对端 < θ 为对应硬负，两端均 < θ 为 neither。主面板建造时，AChE/BChE 与 PIK3CA/PIK3CB 使用严格规则；EGFR/HER2 与 PIK3CA/mTOR 主面板（PM48）使用 θ = 6.0。另在 θ ∈ {5.5, 6.0, 6.5} 与严格规则下重算口袋匹配 `summary_min`，作为标签敏感性对照。

为估计公开数据能否支撑严格四类面板，对 49 对可审计靶对统计严格硬负供给。两端硬负均 ≥ 50 的仅 4 对；去掉金属依赖的 HDAC1/HDAC6 后，适合常规对接厚面板的剩 PIK3CA/mTOR、AChE/BChE、PIK3CA/PIK3CB。EGFR/HER2 在同一规则下严格 B_only 仅 7 个，达不到厚面板门槛。由此冻结 K=4 评价集：上述三对作为严格厚面板席位；EGFR/HER2 作为供给受限案例，沿用既有统一 RDKit 面板，不另做对接扩面。NLRP3/JNK1 保留为私有 holdout，不进入本公开基准。
...
| 审稿人可能问 | 本稿怎么处理 |
|--------------|--------------|
| 为什么 EGFR 也在 K=4，却又说供给不够？ | 角色在 **2.2/2.3** 定义为案例；Results 3.1 只报审计数字，不再写“不做新对接” |
| 标签规则为何不统一？ | **2.2 直接承认** θ=6 vs strict，并说明有统一重算敏感性 |
