...
配体来自同一 ChEMBL 批次，排除主面板成员后按冻结配额抽样。EGFR/HER2 剩余候选不足，未构建同等 holdout，不能解释为已完成重复验证但失败。JAK1/JAK2 实抽 20 / 20 / 18。留出集为方向性样本（dual / A-only / B-only），不含 neither。holdout 区间包含 0 时，解释为尚未获得稳定优势的证据，而不把不显著等同于无优势。
