...
def pair_label(pair):
    return pair + "†" if pair == "PIK3CA/PIK3CB" else pair


def paired_points(ax, x, a, b, colors, labels, offset=0.10):
    """Connected point estimates emphasize a within-category comparison."""
    for xi, ya, yb in zip(x, a, b):
        ax.plot([xi - offset, xi + offset], [ya, yb], color="#B8B8B8", lw=0.85, zorder=2)
    ax.scatter(x - offset, a, s=25, color=colors[0], marker="o", label=labels[0], zorder=4)
    ax.scatter(x + offset, b, s=25, color=colors[1], marker="s", label=labels[1], zorder=4)


def legend_below(ax, ncol=2, fontsize=6.5, y=-0.22):
...
def fig_s2_protocol(D: dict, P: dict) -> None:
    fig, axes = plt.subplots(2, 2, figsize=(7.0, 5.50))
    ...
    for pair_idx, pair in enumerate(PAIR_ORDER):
        ...
        xx = x + (pair_idx - 1.5) * 0.065
        ax.errorbar(
            xx, ys, yerr=yerr, fmt="o", color=PAIR_COLOR[pair], ecolor=PAIR_COLOR[pair],
            elinewidth=0.8, capsize=1.5, markersize=4, lw=1.0, zorder=4, label=pair_label(pair),
        )
        # The strict two-threshold rule is categorical, not a fourth value of theta.
        ax.plot(xx[:3], ys[:3], color=PAIR_COLOR[pair], lw=0.85, zorder=3)
        for xi, yi, u in zip(xx, ys, under):
            if u:
    ...
    ax.scatter(x - 0.18, vina, s=26, marker="o", color=C["vina"], label="Vina", zorder=3)
    ax.scatter(x, m01, s=26, marker="s", color=C["gnina"], label="CNN mode 1", zorder=3)
    ax.scatter(x + 0.18, b9, s=26, marker="^", color="#005F73", label="CNN best of 9", zorder=3)
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks(x)
    ax.set_xticklabels(TICK, fontsize=6.5)
    ax.set_ylabel(r"summary$_{\mathrm{min}}$")
    ax.set_ylim(0, 1.0)
    ax.set_title("CNN rescoring of Vina poses", fontsize=FS_AXIS, pad=3)
    legend_below(ax, ncol=2, y=-0.24)
    ...
    paired_points(ax, x, y48, y110, (C["main"], C["holdout"]), ("PM48", "PM110"))
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks(x)
    ...
    # D: separate axes keep worst-arm summaries distinct from single-target AUROCs.
    ax = axes[1, 1]
    panel_label(ax, "D", x=-0.18, y=1.04)
    e16 = fnum(D["theta6"]["PIK3CA/mTOR"]["pocket_matched_summary_min"])
    e8 = D["e8"]["summary_min"]
    ax.set_axis_off()
    left_ax = ax.inset_axes([0.00, 0, 0.40, 1.0])
    right_ax = ax.inset_axes([0.63, 0, 0.37, 1.0])
    left_ax.scatter([0, 1], [e16, e8], s=29, color=[C["vina"], C["gnina"]], zorder=4)
    left_ax.plot([0, 1], [e16, e8], color="#B8B8B8", lw=0.85)
    right_ax.scatter([0, 1], [fnum(D["enrich"]["4L23"]["auroc"]), fnum(D["enrich"]["4JT6"]["auroc"])], s=29, color=[C["a_only"], C["b_only"]], zorder=4)
    for sub_ax in (left_ax, right_ax):
        sub_ax.axhline(0.5, color=C["chance"], ls="--", lw=0.8, zorder=1)
        sub_ax.set(xlim=(-0.45, 1.45), ylim=(0, 1))
        sub_ax.set_xticks([0, 1])
    left_ax.set_xticklabels(["E = 16", "E = 8"], fontsize=6.5)
    right_ax.set_xticklabels(["4L23", "4JT6"], fontsize=6.5)
    left_ax.set_ylabel(r"summary$_{\mathrm{min}}$")
    right_ax.set_ylabel("AUROC")
    left_ax.set_title("Search effort", fontsize=FS_AXIS, pad=3)
    right_ax.set_title("Single target", fontsize=FS_AXIS, pad=3)
    P["s2D"] = {
        "e16": e16,
        "e8": e8,
        "e8_da": D["e8"]["da"],
    ...
    fig.subplots_adjust(wspace=0.35, hspace=0.58, left=0.09, right=0.98, top=0.94, bottom=0.13)
    save_all(fig, "FigS1_protocol_sensitivity")
    plt.close(fig)


def fig6_wrong_pocket(D: dict, P: dict) -> None:
    """SI S10: point estimates on original, holdout, and matched-subset panels."""
    fig, axes = plt.subplots(2, 2, figsize=(7.0, 5.65))
    w = 0.36
    ...
    paired_points(ax, x, matched, wrong, (C["vina"], "#999999"), ("Matched", "Mismatched"))
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks(x)
    ax.set_xticklabels(TICK, fontsize=6.5)
    ax.set_ylabel(r"summary$_{\mathrm{min}}$")
    ax.set_ylim(0, 1.05)
    ax.set_title("Original panels", fontsize=FS_AXIS, pad=3)
    legend_below(ax, ncol=2, y=-0.24)
    ...
    paired_points(ax, x, hm, hw, (C["vina"], "#999999"), ("Matched", "Mismatched"))
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks(x)
    ...
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks([i + (j - 1) * 0.22 for i in range(len(HOLD_PAIRS)) for j in range(3)])
    ax.set_xticklabels(["U", "P", "S"] * 3, fontsize=6.5)
    for i, lab in enumerate(HOLD_TICK):
        ax.text(i, -0.15, lab, transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=6.5)
    ax.set_ylabel(r"summary$_{\mathrm{min}}$")
    ax.set_ylim(0.15, 1.05)
    ax.set_title("Holdout matching subsets", fontsize=FS_AXIS, pad=3)
    ax.legend(
        handles=[
            Line2D([0], [0], marker="o", color=C["vina"], ls="none", ms=4.5, label="Matched"),
            Line2D([0], [0], marker="s", color="#999999", ls="none", ms=4.5, label="Mismatched"),
        ],
        loc="upper center",
    ...
    ax.set_axis_off()
    contact_ax = ax.inset_axes([0.07, 0, 0.43, 1.0])
    vina_ax = ax.inset_axes([0.68, 0, 0.32, 1.0])
    contact_ax.scatter(cA, x - 0.12, s=24, color=C["a_only"], marker="o", label="Pocket A: D/A", zorder=3)
    contact_ax.scatter(cB, x + 0.12, s=24, color=C["b_only"], marker="s", label="Pocket B: D/B", zorder=3)
    vina_ax.scatter(vw, x, s=25, color="#888888", marker="^", zorder=3)
    for sub_ax in (contact_ax, vina_ax):
        sub_ax.axvline(0.5, color=C["chance"], ls="--", lw=0.8, zorder=1)
        sub_ax.set(xlim=(0, 1), ylim=(2.5, -0.5))
        sub_ax.set_xticks([0, 0.5, 1])
        sub_ax.set_yticks(x)
    contact_ax.set_yticklabels(HOLD_TICK, fontsize=6.5)
    vina_ax.set_yticklabels([])
    contact_ax.set_xlabel("AUROC")
    vina_ax.set_xlabel(r"summary$_{\mathrm{min}}$")
    contact_ax.set_title("Contact count", fontsize=FS_AXIS, pad=3)
    vina_ax.set_title("Vina", fontsize=FS_AXIS, pad=3)
    legend_below(contact_ax, ncol=1, y=-0.25)
    P["fig6D"] = {"contact_A": cA, "contact_B": cB, "vina_wrong": vw}
    P["s4C"] = P["fig6D"]

    fig.subplots_adjust(wspace=0.36, hspace=0.55, left=0.10, right=0.98, top=0.94, bottom=0.17)
    save_all(fig, "FigS10_matched_vs_mismatched_point_estimates")
    plt.close(fig)


def fig_s3_confounds(D: dict, P: dict) -> None:
    fig, axes = plt.subplots(2, 2, figsize=(7.0, 5.80))
    ...
    paired_points(ax, x - 0.18, da_d, da_m, (C["vina"], "#56B4E9"), ("Vina D/A", "ECFP4 D/A"), offset=0.07)
    paired_points(ax, x + 0.18, db_d, db_m, (C["a_only"], C["desc"]), ("Vina D/B", "ECFP4 D/B"), offset=0.07)
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks(x)
    ax.set_xticklabels(TICK, fontsize=6.5)
    ax.set_ylabel("AUROC")
    ax.set_ylim(0, 1)
    ax.set_title("ECFP4 and Vina", fontsize=FS_AXIS, pad=3)
    legend_below(ax, ncol=2, y=-0.24)
    ...
    ax.set_yticklabels(TICK, fontsize=6.5)
    ax.set_xlabel(r"summary$_{\mathrm{min}}$")
    ax.set_xlim(0.05, 0.95)
    ax.set_ylim(-3 * y_gap - 0.45, 0.55)
    ax.set_title("Vina and single descriptors", fontsize=FS_AXIS, pad=3)
    ax.legend(
        handles=[
            Line2D([0], [0], marker="o", color=C["vina"], ls="none", ms=6, label="Vina"),
            Line2D([0], [0], marker="s", color=C["thick"], ls="none", ms=5, label="Heavy atoms"),
    ...
    # C: covariate-adjusted D vs B; this is logistic prediction, not rank AUROC.
    ax = axes[1, 0]
    panel_label(ax, "C", x=-0.18, y=1.04)
    x = np.arange(len(PAIR_ORDER))
    w = 0.32
    ...
    paired_points(ax, x, only, adj, (C["vina"], C["desc"]), ("Score only", "Score + covariates"))
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks(x)
    ax.set_xticklabels(TICK, fontsize=6.5)
    ax.set_ylabel("Dual vs B-only AUROC")
    ax.set_ylim(0, 1)
    ax.set_title("Covariate-adjusted logistic model", fontsize=FS_AXIS, pad=3)
    legend_below(ax, ncol=2, y=-0.24)
    ...
    labs = ["Full panel (not plotted)", "Potency matched", "Size matched"]
    ...
    for i, pair in enumerate(PAIR_ORDER):
        for j, (k, col) in enumerate(zip(kinds, cols)):
            ...
            plotted[k].append(y)
            if k == "full_panel_pocket_matched":
                # This row is a worst-arm summary CI, not a directional D/B CI.
                continue
            ax.errorbar(
                i + (j - 1.5) * 0.20, y, yerr=[[y - lo], [hi - y]],
    ...
    ax.set_ylabel("Dual vs B-only AUROC")
    ax.set_ylim(0, 1.04)
    ax.set_title("Matched subsets", fontsize=FS_AXIS, pad=3)
    ax.legend(
        handles=[Line2D([0], [0], marker="o", color=c, ls="none", ms=5, label=l) for c, l in zip(cols[1:], labs[1:])],
        loc="upper center",
    ...
    P["s3D_source_reference_not_plotted"] = plotted.pop("full_panel_pocket_matched")
    P["s3D"] = plotted

    fig.subplots_adjust(wspace=0.40, hspace=0.58, left=0.11, right=0.98, top=0.94, bottom=0.15)
...
def fig_s2_supply_sampling(D: dict, P: dict) -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(7.0, 3.20))
    ...
    for j, (src, rule, col, lab) in enumerate(series):
        ...
        plotted[lab] = vals
        display_lab = {"BDB equal": "BindingDB (=)", "PubChem equal": "PubChem (=)", "BDB as_is": "BindingDB (all)", "PubChem as_is": "PubChem (all)"}.get(lab, lab)
        ax.bar(x + (j - 2) * bw, vals, bw, color=col, label=display_lab, zorder=3)
    ax.axhline(50, color=C["ink"], ls="--", lw=0.9, zorder=2)
    ax.axhline(20, color="#888888", ls=":", lw=0.9, zorder=2)
    ax.set_xticks(x)
    ax.set_xticklabels(["PIK3CA/\nmTOR", "AChE/\nBChE", "PIK3CA/\nPIK3CB†", "EGFR/\nHER2"], fontsize=6.5)
    ax.set_ylabel("Smaller strict selective class (n)")
    ax.set_ylim(0, 170)
    ax.set_title("Cross-database activity coverage", fontsize=FS_AXIS, pad=3)
    legend_below(ax, ncol=2, fontsize=6.5, y=-0.27)
    P["s4A"] = plotted
    ...
    for j, ((cls, feat), lab, col) in enumerate(zip(feats, feat_lab, feat_col)):
        ...
        plotted[lab] = vals
        display_lab = [r"Dual: pChEMBL$_{A}$", r"A-only: pChEMBL$_{A}$", r"B-only: pChEMBL$_{B}$"][j]
        ax.scatter(x + (j - 1) * 0.18, vals, s=26, color=col, marker=["o", "s", "^"][j], label=display_lab, zorder=3)
    ax.axhline(0.0, color=C["ink"], lw=0.8, zorder=2)
    ax.set_xticks(x)
    ax.set_xticklabels(HOLD_TICK, fontsize=6.5)
    ax.set_ylabel("Mean pChEMBL: holdout − original")
    ax.set_ylim(-2.2, 1.2)
    ax.set_title("Activity shift in holdout compounds", fontsize=FS_AXIS, pad=3)
    legend_below(ax, ncol=2, y=-0.27)
    P["s4D"] = plotted

    fig.subplots_adjust(wspace=0.40, left=0.10, right=0.98, top=0.88, bottom=0.32)
...
def fig_s3_paired_deltas(D: dict, P: dict) -> None:
    ...
    P["s3newA"] = _delta_forest(
        ...
        r"Δ summary$_{\mathrm{min}}$ (matched − mismatched)",
    )
    ...
    P["s3newC"] = _delta_forest(
        ...
        r"Δ summary$_{\mathrm{min}}$ (Vina − best descriptor)",
    )
    ...
    paired_points(ax, x - 0.16, sc_da, rd_da, (C["vina"], "#56B4E9"),
                  ("Scaffold D/A", "Random D/A"), offset=0.065)
    paired_points(ax, x + 0.16, sc_db, rd_db, (C["a_only"], C["desc"]),
                  ("Scaffold D/B", "Random D/B"), offset=0.065)
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9, zorder=1)
    ax.set_xticks(x)
    ax.set_xticklabels(TICK, fontsize=6.5)
    ax.set_ylabel("ECFP4 AUROC")
    ax.set_ylim(0, 1.0)
    ax.set_title("Scaffold- and random-fold estimates", fontsize=FS_AXIS, pad=3)
...
def verify_si(D: dict, provenance: dict, errors: list) -> None:
    ...
    eq(P["s3D_source_reference_not_plotted"][0], 0.4297, msg="s3D EGFR full source")
    ...
    for name in (
        "FigS10_matched_vs_mismatched_point_estimates.png",
