...
**靶对供给筛选。** 靶对宇宙为人源、单组分 SINGLE PROTEIN 靶点的全部无序对（5,869 个靶点；其中 4,672 个至少有一个合格分子；可能无序对 10,911,456 个），不含自身对。至少有一个配体在两端均有定量活性的靶对进入普查（2,164,618 对）。后续门槛使用 ChEMBL 37 转储。两端均至少 10 个配体时剩 63,790 对；\(\theta=6.0\) 下 dual、A-only、B-only 各不少于 10 时剩 5,253 对；严格 6.5/5.5 且两端硬负样本各不少于 50 时剩 86 对。剔除 qHTS 枢纽蛋白、CYP ADME 面板和锌依赖金属酶后剩 26 对。两端各至少 5 个人源全配体结构（\(\leq 3.5\) Å，至少一个非聚合物配体）后剩 19 对。药物类小分子过滤后两端硬负样本仍各不少于 50 时剩 17 对、12 个靶点体系。将新对接限定为适合常规刚性受体 Vina 的可溶小分子口袋——排除膜蛋白 GPCR/转运体、未声明表观遗传结构域和药理学抗靶对——后剩 8 对、6 个体系。

这 8 对中，CTSK/CTSS 因两端共晶均为可逆共价、不属于常规非共价 Vina 而被排除。其余 7 对为 PIK3CA/mTOR、AChE/BChE、F2/F10、JAK1/TYK2、JAK1/JAK2、PPARG/PPARA 和 PPARA/PPARD。EGFR/HER2 作为已声明的供给受限例外纳入：它未过厚供给门槛（严格硬负样本最小值为 7），但具有人源全配体结构、可由共晶定义的对接盒，并在 \(\theta=6.0\) 下有足够 dual、A-only、B-only 配体做方向性评价。这八对即主评价集。通过更早门槛但未对接的靶对不再纳入。
...
跨 DUD-E、LIT-PCBA 及相关基准设定的比较已经表明，对照组成可以改变虚拟筛选评价；[5–7,12,13,20] Kinase-Bench 则面向激酶选择性识别。[21] 本文回答的是另一问题：在已测两端活性的化合物中，更换实验状态比较如何改变对接判别的解释。主要推断来自靶对内固定评分比较，不是八对排行。
...
数据上的主要限制是双端实验测量少且来源异质。四状态分类要求同一配体两端均有测定。尽管统一到 pChEMBL，IC50、Ki、Kd、EC50 和 Potency 在生物学上并不等价。取最大 pChEMBL 进一步偏向最强记录。最大值、中位数和高置信字段筛查是不同检查、覆盖范围也不同（Results 3.5；Table S3），只部分缓解该偏向。所得类别是数据库来源的回顾性状态，不是同一实验体系下的生物学真值。在所用数据来源、过滤规则和准入门槛下，BindingDB 未形成外部评价集。2018 年发表年份子集中仅 JAK1/TYK2 和 JAK1/JAK2 达到报告门槛。

F2/F10 两个方向的 AUROC 均低于随机（Table 2）。这一靶对特异的低于随机结果进一步说明，对接并不在各体系中提供普遍优势。

不同靶对的双向选择性数据供给差异明显，评价面板在候选池和抽样约束上并不完全统一。因此不能把各对的绝对 AUROC 当作同一条件下的比较。主要统计分析统一采用 \(\theta=6.0\) 标签，并进行了留出集和敏感性分析。纳入的八个靶对用于考察不同体系中的一致性，不应视为相互独立的统计重复：JAK1 与 PPARA 各被使用两次，部分靶对属于相近激酶或核受体家族。Figure S5 另有一项样本量情景分析；因其百分位区间的名义覆盖并不理想，仅作为诊断、不作为功效估计（Table S10）。
...
部分回顾性判别可以由不使用受体结构的配体化学获得；在当前 ECFP4 模型和骨架分组划分下，加入对接评分未表现出跨方向一致的 AUROC 提升。七个内部留出集的对应口袋优势未一致再现，因此较高的回顾性 AUROC 本身不足以归因于相应口袋的三维结构信息。对于此类回顾性双靶基准，应分别报告两个方向的选择性任务，并结合配体化学与口袋对应性解释结果。这些指标描述的是已知实验标签集合中的区分能力，不能直接等同于前瞻筛选命中率。最终的双靶活性和选择性仍需实验测定。
