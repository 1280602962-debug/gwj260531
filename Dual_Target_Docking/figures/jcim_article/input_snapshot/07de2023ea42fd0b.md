...
在评价集对接前，对各主受体的共晶配体进行重对接，检查所定义的结合区域和搜索设置能否生成近天然构象。每个共晶配体最多输出 9 个姿态，并计算各保存姿态与实验共晶构象之间的重原子 RMSD。采用全部保存姿态中的最低重原子 RMSD 评估对接搜索对近天然构象的覆盖能力。AChE 4EY7 与 TYK2 3LXP 实际保存 8 个姿态，其余主受体保存 9 个姿态；该指标因此不是统一的 best-of-9。当该值低于 2.0 Å 时，认为搜索能够产生近天然构象。另报告排名第 1 姿态的 RMSD，以区分姿态生成能力和评分排序能力。新增八个受体另用化学对应关系重算 RMSD（Table S2c），近天然判断以该表为准。原有六个受体沿用其已有 CalcRMS 或生产 QC 表，不是同一套新复核。EGFR 3POZ 为后来重建的 QC，不是已找回的原始生产输出。原先仅按坐标匹配的结果仍列于 Table S2b，作为历史对照。对接盒和共晶重对接结果见 Table S2。[8]
