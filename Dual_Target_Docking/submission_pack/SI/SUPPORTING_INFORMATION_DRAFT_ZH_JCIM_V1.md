...
| EGFR/HER2 | 口袋 B（对 A-only） | 0.661 | 0.738 | 0.077 | [−0.128, 0.265] | 否 |
...
**Vina `summary_min` 相对最佳单一描述符的配对 Δ。** 四个单描述符完整矩阵留在仓库，正式 SI 只排每对最佳描述符。AChE/BChE 的 TPSA 方向性 AUROC 为 0.733 / 0.801。八对中六对 95% CI 包含 0；F2/F10 与 JAK1/TYK2 不包含 0。Figure S1 绘制 Vina 区间和描述符点估计，不绘制差值区间。
...
**Figure S4.** (A) 活性阈值敏感性；(B) EGFR/HER2 与 JAK1/TYK2 的簇重采样区间。EGFR/HER2 簇 bootstrap 在规范盒校正后未重算；正式配体层差值为 0.462 [0.262, 0.651]。该图不重复 Figure 4 的留出集口袋对调。
