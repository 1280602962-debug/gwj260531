...
在评价集对接前，对各主受体的共晶配体进行重对接，检查所定义的结合区域和搜索设置能否生成近天然构象。每个共晶配体最多输出 9 个姿态，并计算各保存姿态与实验共晶构象之间的重原子 RMSD。采用全部保存姿态中的最低重原子 RMSD 评估对接搜索对近天然构象的覆盖能力。AChE 4EY7 与 TYK2 3LXP 实际保存 8 个姿态，其余主受体保存 9 个姿态；该指标因此不是统一的 best-of-9。当该值低于 2.0 Å 时，认为搜索能够产生近天然构象。另报告排名第 1 姿态的 RMSD，以区分姿态生成能力和评分排序能力。14 个主受体均用同一套化学对应 CalcRMS 重算 RMSD（Table S2c），近天然判断以该表为准；不重对接。PIK3CA/mTOR 的制备态配体不在晶体坐标系，故对晶体图作自同构 CalcRMS。EGFR 3POZ 为后来重建的 QC，不是已找回的原始生产输出。原先仅按坐标匹配的结果仍列于 Table S2b，作为历史对照。对接盒和共晶重对接结果见 Table S2。[8]
...
共晶重对接用全部保存姿态中的最低重原子 RMSD 判断搜索能否回到近天然构象。14 个主受体现已用同一套化学对应 CalcRMS 复核；接受数字与原先各自表一致，搜索覆盖门槛仍全部通过。这只说明搜索能够产生近天然姿态，不说明第一姿态已被正确排序。EGFR 3POZ 的第一姿态为 9.505 Å，最低保存姿态为 0.760 Å。BChE、mTOR、JAK2、PPARG 和 PPARA 的第一姿态也未回到 2 Å 以内；PPARA 6LXA 前三姿态最低为 7.848 Å（Figure S4；Table S2c）。坐标匈牙利匹配保留在 Table S2b，作为历史对照。
