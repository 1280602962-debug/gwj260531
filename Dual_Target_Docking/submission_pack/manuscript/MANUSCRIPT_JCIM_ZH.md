...
对满足数据供给条件的靶对进一步检查后，部分体系因共同高通量测定背景、靶点类型、结合位点性质或缺少合适的人源实验结构而不适合纳入统一非共价对接评价。最终纳入主要评价的靶对为 PIK3CA/mTOR、AChE/BChE、EGFR/HER2、F2/F10、JAK1/TYK2、JAK1/JAK2、PPARG/PPARA 和 PPARA/PPARD（Table 1）。各靶对的候选池规则、配额和骨架上限按样本供给规模与结构可行性分别设定（Table 1），而不是八个靶对共用一套抽样配额。EGFR/HER2 的严格 B-only 样本供给仍然有限，因此从 \(\theta=6.0\) 候选池建面；主分析 \(\theta=6.0\) 下该对为 28 dual、38 A-only 和 32 B-only。JAK1 同时出现在 JAK1/TYK2 与 JAK1/JAK2，PPARA 同时出现在 PPARG/PPARA 与 PPARA/PPARD。四状态分类要求同一配体在两个靶点上均具有实验测量，双端实验测量覆盖和双向选择性样本数量共同限制了严格四状态评价集的规模。

### 3.2 实验状态定义与方向性对接评价

固定相同评分通道后，改变实验状态比较所产生的 AUROC 差异在不同靶对和评价方向之间差别较大。因此，评价任务差异具有靶对和方向依赖性，而不是“选择性负类一定更难”的普遍效应。其中 EGFR/HER2 和 JAK1/TYK2 出现了较大的差异。EGFR/HER2 使用靶点 A（EGFR）评分时，dual-versus-B-only 的 AUROC 为 0.430，将对照类别替换为双端低活性的 neither 后升至 0.808，差值 0.378 [0.205, 0.547]；另一方向上的差异较小。将该差值的重采样单位由配体改为文献连通簇或 Bemis–Murcko 骨架簇后，区间分别为 [0.083, 0.529] 与 [0.168, 0.562]，仍排除 0。JAK1/TYK2 使用靶点 A（JAK1）评分时，dual-versus-neither 与 dual-versus-B-only 的差值为 0.444 [0.263, 0.620]；骨架簇区间为 [0.234, 0.633]，仍排除 0，但文献簇区间为 [−0.034, 0.682]，包含 0。因此，这两个实例中 EGFR 口袋 A 的任务差值在两种簇重采样下仍然成立；JAK1/TYK2 的同一差值对骨架相关稳健，对文献来源相关更敏感（Table S4；Table S10）。固定评分通道排除了评分汇总方式改变这个因素，但从 B-only 换成 neither 同时更换了具体分子，其骨架、大小、电荷和实验来源也可能变化。因此，现有结果支持“更换对照类别及其对应的化合物集合会改变性能判断”，而不能证明差异已由生物学活性状态本身造成。AChE/BChE、JAK1/JAK2、PPARG/PPARA 和 PPARA/PPARD 在相同评分通道下的差值较小，配体水平 95% 置信区间均包含 0（Figure 2C；Table S4）。
...
在 Bemis–Murcko 骨架分组交叉验证下，不使用受体结构的 ECFP4 分子指纹在多个方向上获得与对接相当甚至更高的 AUROC（Figure 3A）。将对应方向的对接评分作为输入特征加入 ECFP4 逻辑回归模型后，全部 16 个方向上 AUROC 的最大绝对变化为 0.023（Table S5）。因此，仅配体化学是表观对接判别的一个重要竞争解释。加入一个对接分数后增量很小，不等于对接不含结构信息，也不等于所有结构信息都没有增量。

### 3.4 口袋对应性与计算敏感性

如果要把某个 AUROC 解释成口袋特异性结构识别，就应该看到对应口袋优于非对应口袋，并且这种优势至少对样本、受体和实现具有一定稳定性。Figure 5 与 Table S6 比较对应口袋与非对应口袋的 \(\mathrm{summary}_{\min}\) 差值，而不是同一方向的单通道 AUROC 差。仅 EGFR/HER2 和 AChE/BChE 主面板的对应与非对应口袋差值区间排除 0，分别为 0.170 [0.060, 0.280] 和 0.161 [0.037, 0.269]；其余六个靶对的差值 95% 置信区间均包含 0。在未使用池留出集中，七个可评靶对的对应与非对应口袋 \(\mathrm{summary}_{\min}\) 差值均包含 0（差值介于 −0.079 至 +0.150；Figure 5B；Table S7）。留出集区间包含 0，不等于已经证明口袋优势消失；主评价集显著、留出集不显著，也不等于两者差异显著。尤其是同源靶点，非对应口袋也可能产生相关且具有生物学意义的评分。因此，口袋交换是有价值的诊断，但不是证明“没有结构特异性”的决定性实验。更稳妥的结论是：尚未获得稳定的对应口袋优势证据。
...
本研究的结论范围主要受到样本规模、公开活性数据异质性和计算覆盖范围的限制。不同靶对的双向选择性数据供给差异较大，因此评价面板的候选池和部分抽样约束并不完全一致；虽然主要统计分析统一采用 \(\theta=6.0\) 标签，并通过阈值和未使用配体留出集分析考察其影响，但仍不能完全排除面板构建方式对跨靶对比较的影响。八个靶对也不是八个统计上独立的重复：JAK1 与 PPARA 各被使用两次，且若干体系属于相近蛋白家族。八个靶对用于考察效应是否跨体系一致，而不是支持普遍规律的大样本。除 PPARG/PPARA 外，主要 Vina \(\mathrm{summary}_{\min}\) 的配体水平 bootstrap 95% 置信区间均未完全高于 0.5，说明多数靶对的方向性判别仍存在较大统计不确定性。四状态分类又要求同一配体在两个靶点上均有实验测量，而公开数据库中的可用记录在活性指标、实验条件和蛋白构建形式上并不完全一致，也可能更多集中于研究较充分的化学系列。活性阈值、重复记录汇总方式以及高置信记录筛选总体未改变主要观察，但这些分析并不能消除不同实验来源和化学系列造成的数据异质性。共晶配体重对接 QC 只能证明搜索覆盖近天然构象，不能单独证明受体身份或评分排序能力。配体准备未系统枚举质子化状态、互变异构体和未指定立体化学，因此观察到的低性能不能自动全部归咎于对接评分函数本身。
