...
**靶对供给筛选。** 靶对宇宙为人源、单组分 SINGLE PROTEIN 靶点的全部无序对（5,869 个靶点；其中 4,672 个至少有一个合格分子；可能无序对 10,911,456 个），不含自身对。至少有一个配体在两端均有定量活性的靶对进入普查（2,164,618 对）。后续门槛使用 ChEMBL 37 转储。两端均至少 10 个配体时剩 63,790 对；\(\theta=6.0\) 下 dual、A-only、B-only 各不少于 10 时剩 5,253 对；严格 6.5/5.5 双向选择性规则要求 A-only 和 B-only 各不少于 50 个配体时剩 86 对。剔除 qHTS 枢纽蛋白、CYP ADME 面板和锌依赖金属酶后剩 26 对。两端各至少 5 个人源全配体结构（\(\leq 3.5\) Å，至少一个非聚合物配体）后剩 19 对。药物类小分子过滤后每个严格选择性类别仍各保留至少 50 个配体时剩 17 对、12 个靶点体系。其余候选进一步限定为可在统一刚性受体 Vina 流程下表示的常规非共价小分子口袋。膜蛋白 GPCR、转运体、可逆共价位点以及其他需要超出该统一流程处理的体系均被排除；靶对级纳入与排除理由见 Table S14。

经上述流程兼容性限制后剩 7 对：PIK3CA/mTOR、AChE/BChE、F2/F10、JAK1/TYK2、JAK1/JAK2、PPARG/PPARA 和 PPARA/PPARD。CTSK/CTSS 通过了更早的供给与结构门槛，但因两端共晶均为可逆共价而被排除。EGFR/HER2 作为供给受限情形保留：它未满足严格 6.5/5.5 选择性供给标准（最小选择性类别计数为 7），但具有合适的人源全配体结构、可由共晶定义的对接位点，并在 \(\theta=6.0\) 下有足够 dual、A-only 和 B-only 配体做方向性评价。这八对即主评价集。最终结构与流程兼容性门槛的靶对级纳入与排除列于 Table S14。未满足最终流程兼容性标准的靶对未纳入对接评价，相应理由见 Table S14。
