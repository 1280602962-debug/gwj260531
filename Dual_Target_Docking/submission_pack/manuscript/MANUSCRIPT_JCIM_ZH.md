...
在评价集对接前，对各主受体开展共晶配体重对接，以评价搜索设置对实验结合构象的覆盖能力。每个共晶配体最多输出 9 个姿态，并计算各保存姿态与实验共晶构象之间的重原子 RMSD。采用全部保存姿态中的最低重原子 RMSD 评估对接搜索对近天然构象的覆盖能力。AChE 4EY7 与 TYK2 3LXP 实际保存 8 个姿态，其余主受体保存 9 个姿态；该指标因此不是统一的 best-of-9。当该值低于 2.0 Å 时，认为搜索能够产生近天然构象。另报告排名第 1 姿态的 RMSD，以区分姿态生成能力和评分排序能力。最低 RMSD 低于 2 Å 不表示评分排序或选择性预测可靠。新增八个受体另用化学对应关系重算 RMSD（Table S2c），近天然判断以该表为准。原有六个受体沿用其已有 CalcRMS 或生产 QC 表，不是同一套新复核。EGFR 3POZ 为后来重建的 QC，不是已找回的原始生产输出。原先仅按坐标匹配的结果仍列于 Table S2b，作为历史对照。对接盒和共晶重对接结果见 Table S2。[8]
...
共晶重对接用全部保存姿态中的最低重原子 RMSD 判断搜索能否覆盖近天然构象。按各受体对应的重原子 RMSD 计算方法，14 个主受体结构的最低保存姿态 RMSD 均低于 2.0 Å。这只说明搜索能够产生近天然姿态，不说明第一姿态已被正确排序，也不能说明选择性预测可靠。AChE 4EY7 与 TYK2 3LXP 实际保存 8 个姿态，其余主受体保存 9 个姿态。EGFR 3POZ 为后来重建的 QC，不是已找回的原始生产输出；其第一姿态为 9.505 Å，最低保存姿态为 0.760 Å。新增八个受体的化学对应复核中，JAK2、PPARG 和 PPARA 的第一姿态未回到 2 Å 以内；PPARA 6LXA 前三姿态最低为 7.848 Å（Figure S4；Table S2c）。原有六个受体未纳入该新增化学对应复核。坐标匈牙利匹配保留在 Table S2b，作为历史对照。
...
双端实验测量较少且来源异质，是本研究最主要的数据限制。四状态分类要求同一配体在两个靶点上均有实验测量，而公开数据库中的可用记录在活性指标、实验条件和蛋白构建形式上并不完全一致，且往往高度集中于少数研究较充分的药物化学系列。活性阈值敏感性在从严格 6.5/5.5 候选池抽出的六对上变化较小，部分是因为这些配体在多个阈值下保留了相同的主要类别组成；EGFR/HER2 与 PIK3CA/mTOR 的类别组成随阈值变化更明显。最大值、中位数和高置信字段筛查需要分开理解：同日 API 快照只覆盖原有三对，且 EGFR/HER2 的冻结值与 API-max 还包含数据快照差异；AChE/BChE 的 median 与主结果不同；新增五对的转储 max/median 属于另一数据版本（Table S3）。这些检查不能外推为八对上的普遍稳健性证明，也无法消除混合实验来源和化学系列固有的异质性。在所用数据来源、过滤规则和准入门槛下，BindingDB 未形成外部评价集；这不是“客观上不存在外部数据”。在以 2018 年为截止点的内部时间切分中，JAK1/TYK2 和 JAK1/JAK2 满足两个方向的报告门槛，其余六对未达到；该分析不作为外部验证。
