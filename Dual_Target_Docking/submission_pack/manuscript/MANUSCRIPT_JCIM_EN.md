...
Five fixed Vina random seeds produced comparatively limited numerical fluctuation relative to the larger task and receptor effects (Figure 5C; Table S7). EGFR/HER2 used the same corrected cognate-heavy-atom boxes as Table 2; the production seed matches the primary \(\mathrm{summary}_{\min}\) 0.324, and the five-seed range is 0.324–0.350. The EGFR/HER2 fixed-score task difference remained positive on all five seeds. Same-pose RTMScore and GNINA CNN rescoring of PPARG/PPARA are in Table S7. PPARG/PPARA was the only pair whose primary Vina \(\mathrm{summary}_{\min}\) interval lay entirely above 0.5 (0.649 [0.511, 0.746]); RTMScore lowered it to 0.369 [0.237, 0.478], and GNINA CNN rescoring lowered it to 0.500 [0.347, 0.632].
