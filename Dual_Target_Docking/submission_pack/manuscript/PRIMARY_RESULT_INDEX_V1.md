...
| 八对 `vina_mean` Top-10 / AND 过滤 | Results 3.2；Table S10 | Figure 2D / Figure S1 仅为 JAK1/TYK2 示意 | `eight_pair_ranking_operating_point_v1.csv` | `eight_pair_ranking_operating_point_v1.py` | 冻结 `review_scored_membership_v1.csv`；含 neither 的四状态排序 | 固定 k=10，不是固定筛选比例。PIK3CA/mTOR 为 10/48。不是对接质量排行。Figure 2D 仍读两行 `operating_point_examples_review_v1.csv`。 |
