...
正式 SI：`docs/SUPPORTING_INFORMATION_DRAFT_ZH_JCIM_V1.md`、`docs/SUPPORTING_INFORMATION_JCIM_EN_V1.md`（Tables S1–S9；Figures S1–S4）  
...
| 五对转储 max/median | Results 3.5；Table S3 | Table S3 | `five_pair_dump_gated_v1/max_vs_median_auroc_v1.csv` | 五对 dump-gated 脚本 | ChEMBL 37 dump | 已并入八对 dump-gated 检查。仅 PPARA/PPARD 1 个翻转（CHEMBL121）；五对 leftover 仍 MATCH。区间不进入正文。 |
| 八对转储 max/median | Results 3.5；Discussion 4.5；Table S3 | Table S3 | `eight_pair_dump_gated_v1/max_vs_median_auroc_v1.csv`；`eight_pair_dump_gated_v1/parity_v1.csv` | `analyze_eight_pair_dump_gated_v1.py` | ChEMBL 37 dump | 八对生产最大 pChEMBL 与 dump 一致（0 missing / 0 mismatch）。dump-max `summary_min` 三位小数与 Table 2 一致。max→median：EGFR 6/110、AChE 1/95（CHEMBL659）、PPARA/PPARD 1/110（CHEMBL121）。不替代 Table 2。面板构建历史仍不统一。 |
