...
**数字原则：** 下列单元格均从冻结 CSV 读取。刊出值为 CSV 字符串的三位小数舍入（四舍五入，或已排版处的银行家舍入）。末位恰为 5 时两种邻值均可出现（例如 0.5045 写作 0.504 或 0.505）；以 CSV 为准。EGFR/HER2 Table 2 的 TPSA 为 0.4275 → 0.428。逐配体长表、探索性切片与已退出体系的完整记录不进入本 SI，而随代码与 SHA-256 清单归档（Note S14）。  
