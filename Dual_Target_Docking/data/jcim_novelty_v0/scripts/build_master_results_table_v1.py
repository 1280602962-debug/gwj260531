...
def main() -> None:
    ...
    two_pair = [
        {
            ...
            "n_failed": 0,
            "n_dual": pm0["n_dual"],
            "n_A_only": pm0["n_A_only"],
            "n_B_only": pm0["n_B_only"],
            "auroc_D_vs_A": pm0["auroc_D_vs_A"],
            "auroc_D_vs_B": pm0["auroc_D_vs_B"],
            "summary_min": pm0["pocket_matched_summary_min"],
            "ci_lo": pm0["ci_lo"],
            "ci_hi": pm0["ci_hi"],
        ...
        },
        {
            ...
            "delta_vs_original": float(pm_jps["summary_min"]) - pm0_sm,
            "weak_arm": "D_vs_B",
            "failed_ligand": "",
        ...
        },
        {
            ...
            "delta_vs_original": float(pm_dxt["summary_min"]) - pm0_sm,
            "weak_arm": "D_vs_B",
            "failed_ligand": "",
            "note": "Pocket A replaced; pocket B frozen 4JT6. Exhaustiveness 16. Use deposited CSV CIs.",
        },
        {
            ...
            "delta_vs_original": float(pm_jsx["summary_min"]) - pm0_sm,
            "weak_arm": "D_vs_A",
            "failed_ligand": "",
    ...
    ms_path = ROOT / "data" / "jcim_multiseed_v0" / "tables" / "multiseed_auroc_aggregate_v2.csv"
    if ms_path.exists() and ms_path.stat().st_size:
        for r in _read(ms_path):
            rows.append(
                row(
                    block="multiseed_vina_v2",
                    manuscript_table="Table S54",
                    pair=r["pair"],
                    setting=r["metric"],
                    metric="median_across_five_seeds",
                    value=r["median"],
                    note=(
                        f"primary={r['primary_seed_value']}; range={r['min']}-{r['max']}; "
                        "Dual-vs-neither is AUC(vina_mean)"
                    ),
                    source_file="data/jcim_multiseed_v0/tables/multiseed_auroc_aggregate_v2.csv",
                )
            )
