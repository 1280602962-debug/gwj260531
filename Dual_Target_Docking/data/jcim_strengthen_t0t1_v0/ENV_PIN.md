...
原始冻结对接仍由上表环境生成，未在本次修订中重跑。用于重算统计表、置信区间、化学描述符与姿态审计的 Python 环境固定在仓库根目录 `requirements-analysis.txt`：Python 3.12.3、NumPy 2.5.2、pandas 3.0.5、SciPy 1.18.1、scikit-learn 1.9.0、RDKit 2026.03.5。Meeko 0.7.1 与 gemmi 0.7.5 出现在分析依赖列表中，但不把分析 RDKit 版本自动当成历史对接制备版本。bootstrap 子种子使用 SHA-256 稳定偏移，不再依赖跨进程变化的 Python `hash()`。SHA-256 校验和清单是归档完整性记录，不是科学 PASS/FAIL 依据。
