#!/usr/bin/env python3
"""M4-min: re-prep EH40 with RDKit ETKDG+meeko and dock 3POZ/3RCD @ E=8."""
...
PANEL = Path(
    "/home/gwj/repos/gwj260531/Dual_Target_Docking/data/egfr_her2_panel120_v0/tables/panel_v0_120.csv"
)
SEED = 20260727
EXHAUST = 8
N_MODES = 9
...
def write_conf(target: str, lig: str, ligand_pdbqt: Path) -> Path:
    ...
    cx = box["center"][0] if "center" in box else box["center_x"]
    cy = box["center"][1] if "center" in box else box["center_y"]
    cz = box["center"][2] if "center" in box else box["center_z"]
    sx = box["size"][0] if "size" in box else box["size_x"]
    sy = box["size"][1] if "size" in box else box["size_y"]
    sz = box["size"][2] if "size" in box else box["size_z"]
    conf.write_text(
        "\n".join(
            [
                f"receptor = {rec}",
...
def main():
    rows = [r for r in csv.DictReader(PANEL.open()) if r.get("from_panel40") == "yes"]
    print(f"EH40 ligands to prep/dock: {len(rows)}", flush=True)
    for r in rows:
    ...
    results = []
    print(f"dock jobs: {len(jobs)} workers={MAX_WORKERS}", flush=True)
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for fut in as_completed(futs):
            ...
            print(f"[{done}/{len(jobs)}] {res['status']} {res['target']} {res['ligand']} n={res.get('n_modes')}", flush=True)
