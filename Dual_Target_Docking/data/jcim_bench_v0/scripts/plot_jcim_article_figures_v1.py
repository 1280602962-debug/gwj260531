...
def fig5_robustness(D: dict) -> None:
    ...
    ax.set_title("Main panel vs unused-pool holdout", fontsize=FS_AXIS, pad=4)
    ax.legend(
        handles=[
    ...
    ax.set_ylabel("Pocket-matched summary_min")
    ax.set_ylim(0.12, 1.02)
    ax.set_title("PIK3CA/mTOR crystal swap (PM48)", fontsize=FS_AXIS, pad=4)
...
def fig_si_wrong_pocket(D: dict) -> None:
    ...
    ax.bar(x + w / 2, wrong, w, color="#999999", label="Wrong-pocket control", zorder=3)
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9)
    ax.set_xticks(x)
    ax.set_xticklabels(["EGFR/\nHER2", "AChE/\nBChE", "PIK3CA/\nPIK3CB", "PIK3CA/\nmTOR"], fontsize=6.5)
    ax.set_ylabel("summary_min")
    ax.set_ylim(0, 1.02)
    ax.set_title("Main panels", fontsize=FS_AXIS, pad=4)
    ax.legend(loc="upper left", fontsize=6.0, frameon=False)
    PROVENANCE["plotted"]["siA_matched"] = matched
    PROVENANCE["plotted"]["siA_wrong"] = wrong

    ax = axes[1]
    panel_label(ax, "B", x=-0.16, y=1.05)
    hpairs = ["AChE/BChE", "PIK3CA/PIK3CB", "PIK3CA/mTOR"]
    x = np.arange(len(hpairs))
    hm, hw = [], []
    for p in hpairs:
    ...
    ax.bar(x + w / 2, hw, w, color="#999999", label="Wrong-pocket control", zorder=3)
    ax.axhline(0.5, color=C["chance"], ls="--", lw=0.9)
    ax.set_xticks(x)
    ax.set_xticklabels(hpairs, fontsize=6.5)
    ax.set_ylabel("summary_min")
    ax.set_ylim(0, 1.02)
    ax.set_title("Unused-pool holdout", fontsize=FS_AXIS, pad=4)
    ax.legend(loc="upper left", fontsize=6.0, frameon=False)
    PROVENANCE["plotted"]["siB_matched"] = hm
    PROVENANCE["plotted"]["siB_wrong"] = hw

    fig.subplots_adjust(wspace=0.32, left=0.09, right=0.98, top=0.86, bottom=0.18)
    save_all(fig, "FigS1_wrong_pocket_main_vs_holdout")
    plt.close(fig)
