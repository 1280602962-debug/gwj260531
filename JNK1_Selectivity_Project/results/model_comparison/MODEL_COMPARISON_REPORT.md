...
![XGBoost vs Chemprop R² comparison](model_comparison_r2.png)

*Generated by `scripts/plot_model_comparison.py`. Left: 5-fold scaffold CV (XGBoost). Right: holdout R²; when Chemprop 2.0 single-target metrics are missing from `comparison.json`, exploratory Chemprop MTL holdout from `training_report.json` is shown (hatched).*
