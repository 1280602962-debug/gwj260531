...
> **v3.5 更新**: G1–G4 化学策略命名修订——G2 由「新骨架」改为 **「文献 chemotype 相似性较弱」**（Tc 更低、与 G1 不同簇，非结构新颖性分组）；G1/G3 分别统一为「文献活性分子相似性邻近」「酶学校准对照」。  
> **v3.3 更新**: 新增 **§3.5 对接后筛选过程与评判指标**——逐层漏斗、活性/pose/选择性指标区分、两层 pose QC、门槛标定与数据缺口；数据表 `26_对接后筛选漏斗.csv`。  
> **v3.2 更新**: §6.1 增补 **25→16 组内排序键**、**16 人 MD 完整名单**及 G1/G2 shortlist 落选说明（见 §6.1.2、数据表 `25_shortlist_25to16.csv`）。  
...
**为何做 JNK1 选择性筛选**的完整设计逻辑见 **§1.2**；文献中已报道的选择性抑制剂与结构机制见 **§1.4**。

### 1.2 JNK1 选择性筛选的设计分析

本节回答：**为何立项时以 JNK1 亚型选择性为首要目标**，以及计算管线（三亚型模型 + 对接 Δsel + 结构假说）**为何如此分层设计**。下列内容为项目启动时的**设计 rationale**；数据驱动的 pivot 与否定性结论见 §1.3、§4–§5。

#### 1.2.1 生物学与成药动机：为何瞄准 JNK1，而非 pan-JNK

| 维度 | JNK1 作为首选靶点的依据 | 对筛选设计的影响 |
|------|-------------------------|------------------|
| **疾病关联** | JNK1 与 **IPF、NASH、胰岛素抵抗** 等纤维化/代谢病理有较多遗传与药理证据；JNK1 敲除或抑制在多种纤维化模型中显示获益 | 适应症指向**外周、纤维化相关**通路 → 首选 JNK1 而非脑选择性 JNK3 |
| **临床先例** | **CC-90001**（JNK1 功能偏向）由 **CC-930**（pan-JNK / JNK2–3 酶学更强）迭代而来；Bennett 2021 明确表述：纤维化病理中 **JNK1 而非 JNK2/JNK3** 可能是关键驱动 | 提供**成药可行性**先例，支持「JNK1 偏向」而非「任意 isoform」 |
| **pan-JNK 风险** | **SP600125** 等 pan-JNK 工具药广泛用于机制研究，但全家族抑制可能带来**系统毒性**与适应症外效应；CC-930 II 期 IPF 试验因 benefit/risk 终止 | 动机不是「任意 JNK 抑制剂」，而是**在保留抗纤维化效力的同时降低 JNK2/JNK3 相关风险** |
| **新化学先例** | **E1**（JNK1 IC50 = 2.7 nM，JNK2/JNK1 ≈ 7×）、**TCS JNK 6O**、**Q63** 等近年 JNK1 酶学偏向化合物，提供可借鉴 chemotype | 支持在库内寻找 **JNK1 偏好** 而非仅 pan-JNK hit |

**设计结论（立项时）**：治疗假说支持 **JNK1 选择性或 JNK1 功能偏向**；pan-JNK 仅作**活性与对照基准**，不作为最终成药形态。

#### 1.2.2 为何不将 JNK2 或 JNK3 作为首要筛选方向

| 若选 JNK2/JNK3 为主 | 未采纳为主线的理由 |
|---------------------|-------------------|
| **JNK2** | 与免疫、增殖、应激等多效通路相关；文献中 JNK1 与 JNK2 在凋亡等表型上可有**相反作用**（如 UV 应激模型）。项目适应症（纤维化）与 Bennett/CC-90001 脉络一致指向 **JNK1** |
| **JNK3** | 主要 CNS 表达；神经退行性适应症与 IPF/NASH **赛道不同**。结构上做 JNK3 选择性（Leu144/Ile106）文献更成熟，但**与立项适应症不匹配** |
| **JNK2/3 选择性更易做** | §1.4 显示 HR-I（Leu106/Ile106）机制对 **JNK2/3 偏好**证据更强——这是**结构可行性**问题，不改变**生物学首选 JNK1** 的立项逻辑；反而要求在计算上**显式检验**能否找到 JNK1 方向 hit |

**设计结论**：三亚型**均对接、均测 IC50**（湿实验），但**排序与命名**以「相对 JNK1 的偏好」为探索假说，而非追求 JNK2/3 选择性先导。
...
由此确定 **`PROJECT_TECHNICAL_REPORT.md` 核心原则**：

> **活性用 ML 粗筛，选择性用对接（及后续 MD/酶学）验证。**
...
| **L5 裁决** | 同批次 JNK1/2/3 **酶学 IC50** | **唯一** isoform 选择性判定 | — |

**为何 VSW 以 JNK1（3ELJ）分数作主线活性门**：与立项假说一致（优先保留对 JNK1 有计算活性的分子），同时仍对三 isoform 全对接，以便计算 Δsel 并保留三亚型 MD 比较。

#### 1.2.4 结构受体与 benchmark 的设计选型

| 设计选择 | 理由 |
|----------|------|
| **每 isoform 单 PDB、DFG-in**（3ELJ / 3E7O / 3TTI） | Type I 可逆抑制剂与 Enamine 库化学空间匹配；5/5 再对接支持网格可信（§3.2） |
| **排除 3NPC（DFG-out）、8ELC（共价）** | 避免与主线 VSW 结合模式不一致（§3.1、`REFERENCES.md`） |
| **9 个文献 benchmark** | 覆盖 **JNK1 偏好**（E1、TCS JNK 6O）、**JNK2/3 偏好**（CC-930）、**pan**（SP600125）、**共价**（JNK-IN-8）等，用于**标定**而非训练 |
| **Δsel 方向定义** | Δsel = score_JNK2/3 − score_JNK1；与「JNK1 偏好」立项假说同向；需经 benchmark 验证后方可进决策链 |
| **G3 强制采购已知活性物** | 无论 MD 是否通过，保留 **酶学校准尺**（§6、§7） |

#### 1.2.5 选择性相关结构假说的立项依据

在对接之后、MD 之前，项目曾并行尝试以下**与 JNK1 选择性相关**的计算假说：
...
#### 1.2.6 化学库分组（G1–G4）与 JNK1 假说的对应

| 组别 | 设计意图 | 与 JNK1 选择性假说的关系 |
|------|----------|--------------------------|
| **G1** | 文献活性分子相似性邻近（Tc ~0.22） | 检验「相对更接近文献活性分子 chemotype」是否更易出 hit |
| **G2** | 文献 chemotype **相似性较弱**（Tc ~0.12；与 G1 不同簇，**非**新骨架） | 检验**低相似度系列**是否仍能发现可验证 hit（高风险） |
| **G3** | 酶学校准对照（SP600125、CC-90001、CC-930、E1） | 校准 ML/对接/MD 与**已知三亚型 IC50** |
| **G4** | 阴性锚点 | 验证漏斗是否过松 |

湿实验采购 **10 分子** 的设计（§7）由此而来：**不是**「已算出 JNK1 selective hit」，而是**在 JNK1 选择性假说下**，用最小分子数同时检验**管线、化学策略与选择性探索**。

#### 1.2.7 立项时已知风险与 §1.3 pivot 的衔接

立项时即意识到、并在 §1.4 文献调研中进一步确认的风险：

1. **结构难度**：JNK1 选择性化合物在文献中证据弱于 JNK2/3 方向（Ile106 位阻不利于「伸入后袋」型策略）。
2. **数据稀缺**：ChEMBL 明确 JNK1-selective 标签极少，ML 选择性建模统计功效不足。
3. **口袋同源性**：ATP 位点 98% 同一性 → 对接打分差可能落在噪声区（后由 benchmark 43% 方向准确率证实）。

因此项目在设计上预留了 **pivot 路径**：若计算无法可靠分型，则降级为 **pan-JNK 家族结合剂筛选 + 同批次酶学裁决**——即 §1.3 所记录的后期策略。该 pivot **不否定**立项时选择 JNK1 的生物学合理性，只否定「当时选用的计算选择性指标」足以支撑采购决策。

**本节小结**：立项做 JNK1 选择性筛选，是基于 **IPF/NASH 生物学 + CC-90001/E1 成药先例 + pan-JNK 毒性顾虑**；计算上采用 **ML 保活性召回、对接/MD 检选择性假说、酶学终裁** 的分层设计。数据结果表明：**生物学目标仍有效，但 Δsel/Gly87/ML 选择性不能替代湿实验**——故当前报告将终点表述为「10 分子同批次 IC50」，而非「已确认 JNK1 selective hit」。
...
### 1.4 JNK 亚型选择性抑制剂文献背景调研

本节汇总**同时具备**（1）三亚型酶学/生化选择性数据，与（2）小分子结合模式及蛋白残基层面结构解释（共晶、突变或经 SBDD/对接验证）的 JNK 抑制剂文献。目的在于为本项目 benchmark 选择、对接面板设计（§3）、Gly87/Leu106 等策略讨论（§5）及 **§1.2 立项风险** 提供背景，**不**作为本项目已验证的计算结论。
...
#### 1.4.2 共性机制：ATP 口袋内的 isoform 差异残基
...
#### 1.4.4 B 级：有酶学选择性，结构解释以 SBDD/MD 为主
...
#### 1.4.5 C 级与对照：有结构、弱 isoform 选择性
...
**假说**：配体占据 JNK1 特有的小体积 Gly87 邻域，可对 JNK2/3 产生位阻差异 → JNK1 选择性（立项依据见 §1.2.5；机制类比文献中 JNK2/3 选择性化合物的 Leu106/Ile106 差异，§1.4；Bennett et al., *J. Med. Chem.* 2022 [8]）。

**可计算代理**（`gly87_selfcheck_16be.csv`）：`d_Gly87`（配体中心–Gly87 Cα 距离）、`occ_JNK1`（近邻占据）、`predicts_JNK1_selectivity`（启发式组合）。

#### 为什么要测试？

在将 Gly87 作为 MD 硬筛前，用**已知选择性谱的 benchmark** 做回顾性 gate：JNK1 偏好应对应 True，JNK2/3/pan 应对应 False（立项背景 §1.2，文献机制 §1.4）。
...
| 组 | 数量 | 进入 MD（16） |
|----|------|---------------|
| G1 文献活性分子相似性邻近组 | 9 | **4**（690, 2232, 2157, 2389） |
| G2 文献 chemotype 相似性较弱组 | 10 | **6**（2231, 1280, 4795, 2747, **1555, 1762**） |
| G3 酶学校准对照 | 4 | **4**（全部） |
| G4 阴性锚点 | 2 | **2**（全部） |
...
| 步骤 | 键 / 规则 | 用途 |
|------|-----------|------|
| 分组标签 | `chemotype_sim` = max(ECFP4 Tanimoto vs **E1 / Q63 / TCS JNK 6O**) | G1「文献活性分子相似性邻近」vs G2「相似性较弱」（**非**新骨架） |
...
| G2 | 10 | **6** | **2231, 1280, 4795, 2747, 1555, 1762** |
| G3 | 4 | **4** | SP600125, CC-90001, CC-930, E1 |
| G4 | 2 | **2** | **3237, 3411** |

数据来源：`data/shortlist/md_shortlist_final.csv`（2026-07-07 自对接工作区归档）。
...
| 状态 | ID | 组内排序依据（有据部分） |
|------|-----|--------------------------|
| **进 MD** | 2231, 1280, 4795, 2747, **1555, 1762** | 2231：`score_JNK1` 组内最强（−11.22）；1280/4795：off-target pose 备份假说；2747：RMSD 不对称性探索（§6.4.2，未采购）；1555/1762：G2 低相似度系列补充（Tier 2/3，有 HIT 报价但未进 10 人湿实验单） |
| **未进 MD** | **4 个 ID 待归档**‡ | 同组内排序键下排名 **7–10**；完整名单见 `md_shortlist_report_23c8.md` |

**16 人 MD 对接背景**（`md_shortlist_final.csv`；选择性标签**不作** MD 进门依据）：

| ID | 组 | score_JNK1 | Δsel_dock | 进湿实验单 | 备注 |
|----|-----|------------|-----------|------------|------|
| 690 | G1 | −7.76 | +1.08 | ✅ | MD pass_overall；采购 |
| 2232 | G1 | −8.13 | +1.32 | ✅ | MD pass_overall；采购 |
| 2157 | G1 | −8.46 | −1.05 | ✅ | MD pass_overall；采购 |
| 2389 | G1 | −7.59 | −0.23 | ❌ | MD fail（hinge 28.2%）；有报价未采购 |
| 2231 | G2 | **−11.22** | **+3.37** | ✅ | G2 内 JNK1 活性最强；采购 |
| 1280 | G2 | −7.85 | +0.89 | ✅ | JNK2/3 pose 备份；采购 |
| 4795 | G2 | −8.38 | +1.57 | ✅ | JNK2/3 pose 备份；采购 |
| 2747 | G2 | −8.03 | −0.96 | ❌ | RMSD 不对称性最强；有报价未采购 |
| 1555 | G2 | −8.15 | +0.50 | ❌ | G2 低相似度系列 Tier 2；有报价未采购 |
| 1762 | G2 | −8.77 | +0.29 | ❌ | G2 低相似度系列 Tier 3；有报价未采购 |
...
| F2 成药性 | ✅ | 进入 **157** |
| ADMET（F7） | ✅ | 进入 **25** shortlist，归 **G2 相似性较弱组** |
...
| `data/shortlist/md_shortlist_final.csv` | **16 人 MD 完整名单**（对接分、MM-GBSA、ADMET、pose QC） |
| `docs/popular_science/data_tables/25_shortlist_25to16.csv` | 25→16 组内排序与 MD/落选明细（G1/G2 落选 ID **部分待归档**） |
| `docs/popular_science/data_tables/27_MD16_选择性排序与报价.csv` | 16 人 Δsel_dock 排序 + HIT 报价对照 |
...
| 维度 | 内容 |
|------|------|
| **质疑** | SP600125、CC-930 有已知活性但 MD hinge HB < 30%；G2 0/6 可能部分源于**低相似度系列结合模式**与 hinge 指标不匹配，而非完全不结合。 |
