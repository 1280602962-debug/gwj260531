...
> **v3.4 更新**: 入库 **`data/shortlist/md_shortlist_final.csv`**（16 人 MD 完整名单）；补全 G2 进 MD 的 **1555、1762**；新增数据表 **`27_MD16_选择性排序与报价.csv`**（Δsel_dock 排序 + HIT 报价对照）。G1 落选 5 人、G2 落选 4 人 ID 仍待 `candidates_ranked_befe.csv` 归档。  
...
|------|----------|--------------------------|
| **G1** | 相对更接近文献 **JNK1 偏向** chemotype（E1/Q63/TCS JNK 6O） | 检验「已知 JNK1 方向化学空间」是否更易出 hit |
| **G2** | 新骨架、低相似度（Tc ~0.12） | 检验**脱离文献系列**是否仍能发现 JNK1 偏好（高风险） |
| **G3** | 文献对照（SP600125、CC-90001、CC-930、E1） | 校准 ML/对接/MD 与**已知三亚型 IC50** |
| **G4** | 阴性锚点 | 验证漏斗是否过松 |
...
| G2 新骨架 | 10 | **6**（2231, 1280, 4795, 2747, **1555, 1762**） |
| G3 对照 | 4 | **4**（全部） |
| G4 阴性锚点 | 2 | **2**（全部） |
...
| 状态 | ID | 组内排序依据（有据部分） |
|------|-----|--------------------------|
| **进 MD** | 2231, 1280, 4795, 2747, **1555, 1762** | 2231：`score_JNK1` 组内最强（−11.22）；1280/4795：off-target pose 备份假说；2747：RMSD 不对称性探索（§6.4.2，未采购）；1555/1762：G2 新骨架补充（Tier 2/3，有 HIT 报价但未进 10 人湿实验单） |
