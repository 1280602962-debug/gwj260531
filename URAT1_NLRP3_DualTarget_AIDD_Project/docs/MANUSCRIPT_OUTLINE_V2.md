...
### 0.0 正文叙事合同（防“事后改道”读感）

| 实验室过程（可留内部/SI） | 投稿正文应写成 |
|---------------------------|----------------|
| 开发中试过默认引擎 / A vs D | **先验**：对接协议须在 TrueDecoy（主）+ RandomDecoy（否决）上按锁定规则选优，再进入漏斗 |
| redock top-1 未过关 | **先验**：报告三类 RMSD；未达标则限制 pose 主张，不改写为换方法 |
| EGCG 一度像 lead | **设计内**：Pareto ≠ 提名；审计模块的方法学负例 |

一句话 spine 的语序固定为：**问题 → 先验三阶段框架 → 应用与审计 → 假说边界**。禁止时间线复盘“先失败再替换”。
