...
Gout involves hyperuricemia (urate transporter URAT1, SLC22A12) and inflammasome-driven inflammation (NLRP3). We asked whether an asymmetric computational funnel—fast NLRP3 machine learning (ML) on a clinical drug library, followed by open-source structure-based filtering at inward-open URAT1 (PDB 9DKB) and NLRP3 NACHT (PDB 7ALV)—could prioritize repurposing candidates without claiming de novo dual-target discovery. From 8,319 ChEMBL clinical compounds, NLRP3 ensemble classification (scaffold cross-validation AUROC 0.89) yielded 1,588 compounds with P(active) ≥ 0.5; **URAT1 regression ML was not used for library ranking** (benchmark recovery 2/4 under strict thresholds). AutoDock Vina 1.2.5 docking (exhaustiveness = 32) produced dual scores for the merged pool; Pareto analysis on URAT1 docking percentile (S_U) versus max(NLRP3 ML percentile, NLRP3 docking percentile) identified six non-dominated leads in the development dataset. Known URAT1 inhibitors lesinurad and verinurad ranked high on S_U but not on the Pareto front; colchicine showed high ML probability yet poor dual-axis balance, consistent with indirect NLRP3 modulation. URAT1 evidence was independently validated on an 8,973-compound distill set (ROC-AUC 0.705; enrichment factor at 5% = 4.23 on the legacy benchmark panel; to be recomputed under the same Vina protocol), where dotinurad recovered high docking percentile despite ML failure. Spearman correlation between NLRP3 ML scores and 7ALV docking scores within the pool was negligible (ρ ≈ −0.04), supporting a dual-evidence scoring axis. Four 50–100 ns molecular dynamics simulations (benzbromarone and dotinurad @ 9DKB; MCC950 and epigallocatechin gallate @ 9DKB) are planned in GROMACS. The workflow is fully open-source, computationally tractable, and benchmark-consistent; Pareto hits are hypothesis-generating agents requiring literature triage and experimental follow-up.
...
We therefore designed a **dual-evidence funnel**: (i) NLRP3 ML on the full clinical library to reduce chemical space; (ii) parallel **AutoDock Vina** docking at URAT1 9DKB and NLRP3 7ALV on the ML-enriched pool; (iii) Pareto ranking on URAT1 docking percentile versus a composite NLRP3 score; (iv) independent URAT1 retrospective enrichment on a large distill docking set (n = 8,973); and (v) targeted molecular dynamics (MD) on benchmark inhibitors plus one repurposing lead. **URAT1 ML is explicitly excluded from main-screen ranking** because scaffold-split benchmark recovery is incomplete (see §2.2, §4). All docking uses open-source tools for reproducibility without commercial licenses.
...
### 2.2 Machine learning models and their roles

**NLRP3 classifier (prescreen):** An XGBoost ensemble on five NLRP3-related bioassays with Morgan + RDKit descriptors predicted P(active) under scaffold-split five-fold cross-validation (AUROC 0.89, AUPRC 0.91). P(active) ≥ 0.5 defined the **docking pool** (n = 1,588). This model is used **only to shrink the library**, not as the sole NLRP3 ranking metric in the Pareto layer.

**URAT1 regressor (retrospective only):** URAT1 pActivity regression achieved OOF R² = 0.51 and Spearman ρ = 0.73, but **benchmark backtest recovered only two of four scaffold-novel uricosurics** (lesinurad and dotinurad fail at pActivity ≥ 6). We therefore **do not** rank the 8,319 clinical compounds by URAT1 ML. The URAT1 model is retained for the independent 8973 distill enrichment analysis (§2.5) and as a negative control illustrating data asymmetry.

### 2.3 Molecular docking (open source)

All docking was performed with **AutoDock Vina 1.2.5** (no Schrödinger Glide/Maestro). Receptors were prepared from PDB **9DKB** (URAT1, inward-open) and **7ALV** (NLRP3 NACHT, RM5 co-crystal pocket) using gemmi (protein extraction) and Open Babel 3.x (protonation at pH 7.4, rigid receptor PDBQT). Ligands were embedded with RDKit ETKDG and converted to PDBQT with Meeko 0.5.x.

Search boxes were centered on co-crystal ligand centroids (9DKB: 99.97, 102.97, 105.70 Å; 7ALV: 16.76, 35.45, 125.71 Å) with edge lengths 22 Å and 20 Å, respectively (`config/docking_open_source.yaml`). Vina was run with **exhaustiveness = 32** and nine binding modes per ligand; the lowest `minimizedAffinity` (kcal/mol) defined the best pose. Batch execution used `scripts/run_vina_batch.py` with parallel workers.

**Important:** Vina scores are used **only for within-pool percentile ranks** (S_U, S_N^dock); they are not calibrated to experimental K_i and are not interchanged with prior proprietary docking scores. Optional smina local optimization may be applied to top poses (Supporting Information).

Normalized outputs (`dock_score`, `docking_status`, `pdb_id`) were merged with `scripts/merge_docking_pareto.py`.

### 2.4 Dual-target merge and Pareto analysis

For compounds with both URAT1 and NLRP3 scores (n = 1,451), we computed:

- **S_U:** percentile rank of 9DKB Vina score (more negative = better).  
- **S_N:** max( NLRP3 ML percentile, 7ALV Vina percentile ) when `--sn-mode both`.
...
Separately, 8,973 compounds from the distill manifest were re-docked at 9DKB with the **same Vina protocol** for URAT1-only enrichment (active vs decoy labels from curated pActivity). Metrics: ROC-AUC, enrichment factor at 5% (EF@5%), and benchmark percentiles for four uricosurics. This track did not enter NLRP3 ML or Pareto merging.

### 2.6 Planned molecular dynamics

Four systems (50–100 ns, **GROMACS**):  
**URAT1 @ 9DKB:** benzbromarone, dotinurad (Vina poses; not in P≥0.5 pool).  
**URAT1 @ 9DKB (repurposing lead):** epigallocatechin gallate (EGCG; Pareto nominee).  
**NLRP3 @ 7ALV (optional SI):** MCC950 redocked to the RM5 template pocket.  
Analysis: backbone RMSD, key residue distances (URAT1 Phe cage; NLRP3 Walker B / sulfonylurea pocket), qualitative MM-PBSA.

---

## 3. Results

> **Note on docking scores:** Pareto tables below reflect the **funnel architecture and benchmark behavior** validated during method development. Final submission numbers will use **Vina-only** percentiles after batch re-docking (`docs/OPEN_SOURCE_DOCKING.md`). Qualitative conclusions (colchicine excluded, lesinurad non-Pareto, six-front shortlist) are based on within-pool ranking and are expected to be robust to rescoring.

### 3.1 Asymmetric data justify the funnel design

URAT1 regression ML on curated ChEMBL data recovered only two of four benchmark uricosurics under strict activity thresholds, whereas NLRP3 classification achieved AUROC 0.89 on scaffold splits. **This disqualifies URAT1 ML as a standalone clinical-library ranker** and motivates docking-led S_U. Zero compound overlap exists between the 8973 distill set and the 8,319 clinical manifest, mandating parallel URAT1 retrospective validation rather than a single merged training table.

### 3.2 NLRP3 ML prescreen (Fig. 2)

Of 8,319 compounds, 1,588 (19.1%) exceeded P(active) ≥ 0.5. Gout co-medications allopurinol, febuxostat, benzbromarone, and dotinurad showed low NLRP3 probabilities (P ≈ 0), as expected for non-inflammasome mechanisms. Conversely, colchicine and verinurad scored high (P ≈ 0.92), illustrating phenotypic and off-target confounding. Phase composition skewed toward Phase I/II among high-scoring compounds (Discussion). After dual docking, **1,451** compounds retained both target scores (137 pool members lacked 9DKB and/or 7ALV poses).

### 3.3 URAT1 8973 retrospective (Fig. 3)

On subset A (n = 822 actives) versus subset D decoys (n = 7,957 docked), 9DKB docking achieved ROC-AUC **0.705** and **EF@5% = 4.23** (development benchmark; Vina recomputation pending). Dotinurad ranked ~89th percentile by docking but ~5th by ML pActivity, demonstrating that **URAT1 ranking in this project must be docking-led**. Lesinurad and verinurad showed moderate-to-high docking recovery; benzbromarone was well recovered on 8973 but excluded from the NLRP3 funnel by design.
...
Spearman ρ(P(active), 7ALV Vina score) = **−0.036** (p = 0.17) across the merged set, indicating weak linear coupling between ML prescreen and structural NLRP3 scores.
...
**Model quality (anticipated reviewer questions):** We do not claim both ML models are high-performance single-target screens. URAT1 ML fails two of four external uricosuric benchmarks; NLRP3 ML is adequate for library reduction but mis-ranks colchicine. The funnel is designed so that **weak URAT1 ML cannot dominate outcomes** (docking-led S_U) and **NLRP3 ML confounding is filtered** (colchicine fails Pareto; S_N uses max(ML, dock)). See `docs/REVIEWER_MODEL_QUALITY.md`.

**Open-source docking:** Migration from proprietary Glide XP to Vina ensures reproducibility without Schrödinger licenses. Search exhaustiveness 32 approximates Glide SP-level sampling; absolute kcal/mol values are not compared across engines—only within-engine percentiles.

**Limitations:** (i) 204 pool compounds missing dual scores; (ii) Vina scores are pose hypotheses, not affinities; (iii) no experimental validation; (iv) benchmark uricosurics outside P≥0.5 cannot appear on the main Pareto plot; (v) MD section pending; (vi) full Vina batch on 1,588 × 2 targets is computationally intensive (~CPU-weeks on a workstation).
