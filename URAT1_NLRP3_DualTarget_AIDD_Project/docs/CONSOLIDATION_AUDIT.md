...
| “固定模型哈希即可证明1588可重算” | 原summary无同时生成的模型哈希；重训6.0池1377 | 冻结池是成员依据，模型历史来源链标记未证实 |
...
1. 补齐历史模型文件、环境和1588分数生成链的证据，或保留当前透明的冻结结果复现范围。不能仅把1377差异归因于多线程而声称原因已验证。
2. 酸等价物匹配器范围和历史药名排除应在投稿前决定是否接受为明确限制；若修订，需版本化重算，不能原地改12/21。
3. 逐任务GNINA版本与硬件日志需要汇总；旧文档出现1.3.1/1.3.2，参数相同不能证明所有二进制行为等同。当前统一配置不回填不存在的历史版本证据。
4. PF提名仍为结构后续优先级。MR背景文献、无命中文献检索、厂商SMILES和靶器官推断应回到原始文献核验；不能由“原肾病适应症”直接推断URAT1顶膜暴露，也不能由文献零命中宣称新靶点已确认。
5. MD尚未执行；力场、体系准备、平衡、seed、分析窗口与数值对照失败规则未完全锁定。没有轨迹数字可写。MM/GBSA仅留后续支持分析目录。
