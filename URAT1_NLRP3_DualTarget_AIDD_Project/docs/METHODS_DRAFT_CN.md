...
NLRP3 相关细胞活性筛选后的分子分别在 9DKB 和 7ALV 上按选定协议对接。对接池为满足 \(q_N\ge0.5\) 的 1,588 个分子。百分位与 Pareto 审计只在同时获得两靶有效 gnina P2 分数的完整案例子集上进行；该子集在生产漏斗完成并归档后填入。由于不同靶点和评分指标的原始数值不具有直接可比性，所有有效分数均在该交集内转换为平均秩百分位。令 \(\mathrm{PR}(x_i)\) 表示 \(x_i\) 的平均秩除以有效分子数，取值范围为 0–1，并列值采用平均秩。统一为高值优方向后，三个证据分量定义为
