...
各读出在 TrueDecoy 与 RandomDecoy 上分别计算 ROC-AUC、EF\(_{1\%}\) 和 EF\(_{5\%}\)。由于目标是虚拟筛选前端的早期检出而非全库排序，TrueDecoy 的 EF\(_{1\%}\) 设为首要判据；结果相同时依次比较 EF\(_{5\%}\) 和 ROC-AUC。RandomDecoy 作为否决对照：若某读出仅在 TrueDecoy 上占优而在 RandomDecoy 上早期命中为零，则不选为生产协议。因此生产读出不是“TrueDecoy EF\(_{1\%}\) 最高者”，而是在独立基准上同时满足早期富集与随机诱饵非失效的读出。各读出另用超几何检验评估 TrueDecoy 前 1% 命中是否高于随机。补充材料对已发表 hits@1% 给出 Clopper–Pearson 95% EF 区间；因分子级协议排名未归档，该区间不是 ranking bootstrap，也不报告 AUC 置信区间。选定后，同一配体准备、搜索深度和评分设置应用于 9DKB 与 7ALV。该迁移提供统一生产框架，并不声称该读出已在 NLRP3 诱饵集上独立优化。临床库生产筛选采用 gnina CNNaffinity（P2），exhaustiveness 为 32，并启用 CNN rescoring；每个配体按其首选构象的 CNNaffinity 进入后续百分位排序。P2 在自对接中可以采样到近原生姿，但第一构象不一定通过 2.0 Å 门控，因此不以该读出的 Top-1 作为构象金标准。具体富集数值与自对接 RMSD 见结果部分。

### 2.7 双靶百分位排序与 Pareto 审计

NLRP3 相关细胞活性筛选后的分子分别在 9DKB 和 7ALV 上按选定协议对接。对接池含 1,588 个分子；同时获得两靶有效分数的交集为 1,451 个（约 8.6% 未进入百分位表）。在可复现的配体准备探测中，137 个缺失分子中 7 个无法生成 PDBQT（构象嵌入失败或多片段盐型），其余 130 个可以准备但不见于归档的双成功合并表；仓库不含逐分子对接日志，故不把后者拆成 9DKB 或 7ALV 的引擎错误码。缺失集分子量明显高于完整案例（中位 616 Da 对 473 Da）。失败并非完全随机缺失，百分位分析属于完整案例分析。由于不同靶点和评分指标的原始数值不具有直接可比性，所有有效分数均在该交集内转换为平均秩百分位。令 \(\mathrm{PR}(x_i)\) 表示 \(x_i\) 的平均秩除以有效分子数，取值范围为 0–1，并列值采用平均秩。统一为高值优方向后，三个证据分量定义为
