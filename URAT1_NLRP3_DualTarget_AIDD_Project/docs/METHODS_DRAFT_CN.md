...
本研究的总体流程如图 1 所示。鉴于 URAT1 与 NLRP3 的数据基础不同，两类证据在筛选流程中承担不同功能：NLRP3 分类模型用于缩小临床化合物库，URAT1 则主要依靠结构对接完成排序。为避免临床库中的候选身份影响对接方法的选择，我们先在 URAT1 活性物–诱饵基准上比较不同搜索与打分组合；协议选优不使用临床库排名结果，并在解释临床库筛选结果之前完成。随后，NLRP3 模型筛得的分子分别对接至 URAT1 和 NLRP3；结构分与 NLRP3 模型分转换为池内百分位后做 Pareto 非支配分析（审计用）。跟进提名另设双对接百分位门控（\(S_U\) 与 \(S_{N,\mathrm{dock}}\) 同时达到阈值），再经结构警报、类药性、分子量窗口与骨架去冗余，以区分数学非支配分子与较高优先级假说。URAT1 回归模型仅作对照，不参与临床库主排序。生产对接协议为事先锁定的 Π\*（gnina CNNaffinity，P2）。
...
URAT1 活性基准由清洗数据中 \(\mathrm{pActivity}\ge6\) 的分子组成（469 个）。TrueDecoy 负样本由两部分组成：（i）清洗数据中 \(\mathrm{pActivity}<5\) 的实验弱活性/低活性分子（80 个），作为有实测依据的阴性种子；（ii）自商业类药库（taosu 百万级库经水库抽样与物化包络预过滤后的候选池，约 6 万条）抽取的未标注分子，经分子量、\(\log P\)、拓扑极性表面积、氢键供体数、氢键受体数和可旋转键数的性质匹配补足，使活性物–诱饵比例达到 1:10。对活性分子 \(a\) 与候选匹配诱饵 \(x\)，归一化性质距离定义为
...
性质匹配诱饵采用不重复的 round-robin 方式分配，随机种子固定为 42；在扩大后的候选池上，全部 4,610 条匹配均落在严格窗口内，无需放宽窗口。该构建参照 Gu 等双诱饵评测框架，但负样本并非 BindingDB 多靶实验无活集，而是单靶 URAT1 上的“实验弱活种子 + 商业库性质匹配分子”。最终 TrueDecoy 含 469 个活性分子与 4,690 个负样本（80 个实验弱活 + 4,610 个性质匹配），比例为 1:10。

RandomDecoy 与 TrueDecoy 共用同一批活性分子。负样本按 Gu 文思路从同一预过滤商业库池中**随机**抽取，数目与 TrueDecoy 负样本相同（4,690），且**禁止**与 TrueDecoy 负样本 SMILES 重叠；不得回退使用已被 TrueDecoy 占用的分子。两套诱饵集合的 SMILES 交集为零。对接时对分子并集（`unique_docking_pool.csv`，9,849 个独特结构）只计算一次，再分别汇总 TrueDecoy 与 RandomDecoy 上的富集指标。早期基于蒸馏子集 D（约 8,000）且允许诱饵重叠的版本已废弃，不以该旧集作为协议筛选依据。
...
候选提名在 Pareto 分析之后独立进行，且明确与“仅按对接分取 Pareto 前沿”区分：原始 Pareto 短名单保留作审计，跟进分子不以对接分为唯一排序依据。首先保留同时满足 \(S_U\ge90\) 和 \(S_N\ge90\) 的双轴高分集合；对集合内每个分子标注是否命中 PAINS 或 Brenk、是否同时通过 Lipinski 与 Veber 条件、分子量是否落在口服小分子窗口 \(200\le\mathrm{MW}\le550\) Da、以及 NLRP3 对接百分位是否达到 90。无 PAINS、无 Brenk，且同时通过 Lipinski 和 Veber 条件者标记为清洁候选（clean candidate）；在此基础上进一步满足口服分子量窗口与 Veber/口服吸收替代标志者标记为优选候选（preferred candidate）。原始双阈值集合仍完整保留，以便报告大环内酯类等高分子量对接优势分子被降级的原因。已知对照药另作标记，不与新重定位候选混为“新命中”。排序时依次优先考虑优选候选、清洁候选、分子量窗口、非已知对照药、NLRP3 结构证据支持、较高临床阶段、较高的双结构平衡分
...
以及由 QED、分子量居中性、Ghose 通过与羧酸软标记构成的化学排序分；最后按 Murcko 骨架贪心去冗余生成跟进短名单，避免同一大环骨架占据多个名额。该提名规则不把对接绝对分或未经验证的线性加权总分作为主排序，因而能够保留每个候选的结构证据来源和降级原因。
