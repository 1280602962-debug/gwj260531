...
### 2.5 闸门结果（2026-09-05，用户本机回报）

苯溴马隆 @ 9DKA × seeds 42/43/44：**未通过**。CNNscore Top-1 RMSD ≈ **3.59–3.60 Å**（阈值 2.0）。其余 29 格、W4、W2 **正确未开**。

**现在不准做的：** 把阈值改成 4 Å、把 3.6 当成及格、把其余 29 格当补救、用失败自对接姿去标 W2。

**现在只做审计**（先对已有 3 个 SDF，零新对接）：逐种子报 9 个模式的 RMSD（Top-1 / Top-3 最小 / best-of-9）；核对盒子是 9DKA `[107.167, 105.296, 107.628]` 不是 9DKB；核对准备后受体里没有留下 R75；核对配体是冻结酚负离子 PDBQT；RMSD 必须是对称校正重原子。文件没问题才允许两个诊断性 gnina：酚 vs 酚负离子 seed42、从晶体 R75 做 `local_only` 最小化。

审计后三岔：

| 审计结论 | 下一步 |
|---|---|
| 盒子/受体/RMSD 算错 | 只重跑闸门 3 个种子 |
| best-of-9 ≤ 2.0，只是 CNNscore 选错姿 | 闸门仍判失败（不改 2.0）。W2 改从**晶体坐标**锚定 IFP，不用失败自对接姿。7ALV 已通过，W4 可以开。29 格不当通过条件 |
| 晶体姿 `local_only` 也站不住 | URAT1 自由对接故事停。只用晶体 IFP。29 格不要跑 |
