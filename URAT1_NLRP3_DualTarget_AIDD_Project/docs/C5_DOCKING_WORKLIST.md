...
**分岔：搜得到、选不中。** 9 姿里有晶体盆（~1.11 Å），CNNscore 第一姿是口袋内另一取向（~3.59 Å）。与 lesinurad@9DKB 同类。

不要用审计报告里的 GetBestRMS（≈1.65 / 0.48 Å）把闸门改成通过：`GetBestRMS` 会先叠合配体，那是构象相似度，不是对接姿态 RMSD。2.0 Å 门槛仍作用在**未叠合**的 Top-1 `pose_rmsd` 上。
