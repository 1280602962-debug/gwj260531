...
对照近半年 *Mol. Divers.* 虚筛文（逐篇方法与 DOI 见 **`docs/C5_RANKING_AND_NEXT_DOCKS.md`** §3）：无湿对照是 Klebsiella PBP3+β-内酰胺酶萜类双靶、µ 阿片偏倚激动剂、PCOS 3β-HSD1、M3 抗毒蕈碱。**CERS2 那篇有酶活/LC-MS/MS，不是无湿文**，不得再和它们并列。过审靠五件事：具名候选、疾病/靶点钩子、漏斗厚度、**化学成分**、可复现外观。本项目全部具备，且多出同行普遍没有的两项：

1. **两臂门控各自有回顾性富集统计**（同行几乎只做"对接分→MD 稳定→点名"的自指闭环，且对接分仍是他们的排序器）；
2. **多种子稳定性 + 明确的对照失败停规则**（URAT1 上对接分排序已关闭，见同文档 §2 / §4）。
...
**2026-09-05 闸门已跑完并审计**：苯溴马隆@9DKA 三种子 Top-1 `pose_rmsd` ≈ 3.59 Å（失败），best-of-9 ≈ 1.11 Å（搜得到、选不中）。准备 QC 通过。闸门**不改判**。W1 其余 29 格改为可选 SI。W4 与 W2 按分岔授权：W4 无科学依赖；W2 改锚定沉积晶体坐标。详见 `docs/C5_RANKING_AND_NEXT_DOCKS.md` 与工单「现在要开的对接」。
...
**修正后的标定方法（锚定法，不调参；2026-09-05 再修正锚点来源）**：NLRP3 门可以锚定在自对接之下，因为 NP3-146 自对接重现晶体（Top-1 RMSD 0.67–0.82 Å）。URAT1 自对接选姿失败，故 URAT1 侧四个阈值改为从**沉积晶体配体坐标**（R75 / A1AIL / A1A45 的 PDB/CIF 原始坐标）读 overlap/IFP/接触，取保守下界，**不用** CNNscore 选出的重对接姿态。锚定后**只评估一次** 228 vs 64 回顾集，不回头调整。
