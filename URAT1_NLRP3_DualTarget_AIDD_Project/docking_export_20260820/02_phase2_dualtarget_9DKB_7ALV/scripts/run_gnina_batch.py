...
def run_batch(
    ...
    tasks = []
    cached: dict[int, tuple[float | None, str, str]] = {}
    for i, (_, row) in enumerate(manifest.iterrows()):
        rid = row["repurposing_id"]
        lig = row["pdbqt"]
        out_sdf = poses_dir / f"{rid}_out.sdf"
        log_path = out_sdf.with_suffix(".log")
        # Resume: reuse finished poses (non-empty SDF + parseable log)
        if out_sdf.exists() and out_sdf.stat().st_size > 0 and log_path.exists():
            score, status = parse_gnina_log(log_path.read_text(errors="ignore"), score_mode)
            if status == "docked" and score is not None:
                cached[i] = (score, status, str(log_path))
                continue
        tasks.append(
            (
                i,
    ...
    results_by_idx: dict[int, tuple[float | None, str, str]] = dict(cached)
    if tasks:
        if jobs <= 1:
            for idx, t in tasks:
                results_by_idx[idx] = _worker(t)
        else:
            with ProcessPoolExecutor(max_workers=jobs) as ex:
                futs = {ex.submit(_worker, t): idx for idx, t in tasks}
    ...
    for i, (_, row) in enumerate(manifest.iterrows()):
        rid = row["repurposing_id"]
        out_sdf = str(poses_dir / f"{rid}_out.sdf")
        score, status, log_path = results_by_idx[i]
        records.append(
            {
                "repurposing_id": rid,
