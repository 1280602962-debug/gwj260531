...
without re-docking. Relaxes the thin Pareto front to a dual percentile gate, then
applies transparent chemistry filters so cleaner dual-node candidates surface —
not docking-score giants (macrolides / polyketides) that inflate contact scores.

IMPORTANT revision for dual NLRP3+URAT1 inhibitor claims
--------------------------------------------------------
The original Pareto used sn_mode=both = max(ML, dock). That lets macrolides /
ML-only compounds dominate. For dual-*structure* nomination this script defaults
to a dual-DOCK gate:
...
Selection funnel (on EXISTING dual-dock + ML percentile data):
  ...
  3. Drug-likeness    : Lipinski pass AND Veber pass
...
def main() -> None:
    ...
    # 1. Dual gate — dual_dock is the reviewer-defensible default for dual inhibitors
